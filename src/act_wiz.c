...
char *colorize(char *str)
{
	...
	while(*str != '\0')
	{
		...
		sprintf(buf + strlen(buf), "{k%c", *str);
		str++;
	}
	sprintf(buf + strlen(buf), "{x%c", '\0');

	return buf;
}

char *fixify(CHAR_DATA * ch, char *str)
{
        static char buf[MAX_STRING_LENGTH * 2];
	static char nbuf[MSL * 2];
...
char *hackify(CHAR_DATA * ch, char *str)
{
	static char buf[MAX_STRING_LENGTH * 2];

	buf[0] = '\0';
	strcpy(buf, hacklang(ch, str));
	return buf;
}


void do_roomnames(CHAR_DATA *ch, char *argument)
{
	...
	buf[0] = '\0';
	for( vnum = i = start-1; i < start+50; i++ )
	{
		do
		{
			vnum++;
			room = get_room_index(vnum);
			if(room && word && strstr(room->name,argument) == 0)
				room = 0;
		} while(!room && vnum < 50000);

		if(vnum >= 50000)
			break;

		sprintf(buf+strlen(buf),"(%-5li) -- %s\n\r",vnum,room->name);
	}
	send_to_char(buf,ch);
}


void do_findbest(CHAR_DATA *ch, char *argument)
{
	char buf[MAX_STRING_LENGTH];
	...
	{
		for(objindex = obj_index_hash[i]; objindex; objindex = objindex->next)
		{
			if(is_artifact(objindex->vnum) || is_relic(objindex->vnum))
				continue;

			if(objindex->item_type == ITEM_ARMOR && objindex->value[0] > aca)
			{
				aca = objindex->value[0];
				ac = objindex->vnum;
			}
			else if(objindex->item_type == ITEM_WEAPON && ((objindex->value[1]+objindex->value[2])/2) > wpna)
			{
				wpna = ((objindex->value[1]+objindex->value[2])/2);
				wpn = objindex->vnum;
			}

			for(paf = objindex->affected; paf; paf = paf->next)
			{
				if(objindex->item_type == ITEM_ARMOR && paf->location == APPLY_HITROLL && paf->modifier > hita)
				{
					hita = paf->modifier;
					hit = objindex->vnum;
				}
				else if(objindex->item_type == ITEM_ARMOR && paf->location == APPLY_DAMROLL &&
paf->modifier> dama)
				{
					dama = paf->modifier;
					dam = objindex->vnum;
				}
				else if(paf->location == APPLY_HIT && paf->modifier > hpa)
				{
					hpa = paf->modifier;
					hp = objindex->vnum;
				}
				else if(paf->location == APPLY_AC && paf->modifier < (0-aca))
				{
					aca = paf->modifier*-1;
					ac = objindex->vnum;
				}
				else if(paf->location == APPLY_MANA && paf->modifier > manaa)
				{
					manaa = paf->modifier;
					mana = objindex->vnum;
				}
				else if(paf->location == APPLY_MOVE && paf->modifier > movea)
				{
					movea = paf->modifier;
					move = objindex->vnum;
				}
	...
	sprintf(buf,		"Object Index with highest:\n\r");
	sprintf(buf+strlen(buf),"   Hitroll: %li--(%li)%s\n\r",hita,hit,get_obj_index(hit)->short_descr);
	sprintf(buf+strlen(buf),"   Damroll: %li--(%li)%s\n\r",dama,dam,get_obj_index(dam)->short_descr);
	sprintf(buf+strlen(buf),"        AC: %li--(%li)%s\n\r",aca,ac,get_obj_index(ac)->short_descr);
	sprintf(buf+strlen(buf),"Weapon Avg: %li--(%li)%s\n\r",wpna,wpn,get_obj_index(wpn)->short_descr);
	sprintf(buf+strlen(buf),"        HP: %li--(%li)%s\n\r",hpa,hp,get_obj_index(hp)->short_descr);
	sprintf(buf+strlen(buf),"      Move: %li--(%li)%s\n\r",movea,move,get_obj_index(move)->short_descr);
	sprintf(buf+strlen(buf),"      Mana: %li--(%li)%s\n\r",manaa,mana,get_obj_index(mana)->short_descr);
...
void do_colorize(CHAR_DATA * ch, char *argument)
{
	...
	if(str_cmp(ch->name, "Pip") && str_cmp(ch->name, "Borlak"))
        {
                if(ch->pcdata->colr == 1)
                {
                        ch->pcdata->colr = 0;
                        send_to_char("Whew... things are back to normal now!!\n\r", ch);
                        return;
                }
                send_to_char("0h wh4+ 1n d4 h3<k!!!\n\r", ch);
                ch->pcdata->colr = 1;
                return;
        }
	...
	{
		crazy = 0;
		do_echo(ch, "\n\rThings seem to be back to normal...\n\r");
	}
	else
	{
		crazy = 1;
		do_echo(ch, "\n\rSomething VERY wierd just happened...\n\r");
	}
	return;
}


void do_hackify(CHAR_DATA * ch, char *argument)
{
	if (IS_NPC(ch))
		return;
	...
	{
		hackified = 1;
		do_echo(ch, "\n\rOh no, something is wrong\n\r");
	}
	return;
}


void do_mmvamp(CHAR_DATA * ch, char *argument)
{
	...
	if(!IS_NPC(victim) && victim->pcdata->class == 0)
	{
...
void do_wizhelp(CHAR_DATA * ch, char *argument)
{
	...
	for(cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++)
	{
		...
		{
			sprintf(buf, "{y[%-2li]{Y%-12s", cmd_table[cmd].level, cmd_table[cmd].name);
			send_to_char(buf, ch);
			if(++col % 4 == 0)
				send_to_char("\n\r{x", ch);
		}
...
void do_repeat(CHAR_DATA *ch, char *argument)
{
	...
	if( !is_number(arg1) )
	{
		send_to_char("Repeat <how many times> [command and arguments]\n\r",ch);
		return;
	}
...
{
	insult_update(argument);
}
void do_maketoken(CHAR_DATA * ch, char *argument)
{
	if(argument[0] == '\0')
	{
		stc("Syntax: maketoken yes (type) (amount)\n\r", ch);
		stc("TYPES: quest,primal,exp,weapon,spell,stance,hp,mana,move,random\n\r", ch);
		return;
	}
	token_update(argument);
}

void do_code(CHAR_DATA * ch, char *argument)
{
	code_update(argument);
}
...
void do_deimm(CHAR_DATA *ch, char *argument)
{
	...
	if( argument && str_cmp(arg1,"diebitch") )
	{
		send_to_char("Do that again and you will be deleted :-)\n\r",ch);
		return;
	}

	if( (victim = get_char_world(ch,arg2)) == NULL )
	{
		send_to_char("Deimm who?\n\r",ch);
		return;
	}

	if( !IS_IMMORTAL(victim) )
	{
		send_to_char("Why deimm them?  They are a puny mortal!\n\r",ch);
		return;
	}

	victim->level = 3;
	victim->trust = 3;
	send_to_char("Looks like you have been a bad boy :(  Someone has de-imm'd you!\n\r",victim);
	send_to_char("Done, oh powerful dude!\n\r",ch);
	save_char_obj(victim);
}

void do_godgift(CHAR_DATA * ch, char *argument)
{
	char buf[MAX_STRING_LENGTH];
	CHAR_DATA *victim;
...
*/
	...
	act("{BYou have been blessed by $n!{x", ch, 0, victim, TO_VICT);

	sprintf(buf, "{B%s has been blessed by %s!{x", victim->name, ch->name);
	do_info(ch, buf);
	return;
}

void godgift(CHAR_DATA * victim)
{
	AFFECT_DATA af;
...
void do_godcurse(CHAR_DATA * ch, char *argument)
{
	char buf[MAX_STRING_LENGTH];
	CHAR_DATA *victim;
...
*/
	...
	act("{RYou have been cursed by $n!{x", ch, 0, victim, TO_VICT);

	sprintf(buf, "{R%s has been cursed by %s!{x", victim->name, ch->name);
	do_info(ch, buf);
	return;
}

void godcurse(CHAR_DATA * victim)
{
	AFFECT_DATA af;
...
void do_scatter(CHAR_DATA *ch, char *argument)
{
	...
	for( obj = room->contents; obj; obj = obj_next )
	{
		...
		act("$p dissapears!",ch,obj,NULL,TO_ROOM);
		obj_from_room(obj);

		do
		{
			to_room = get_room_index(number_range(1,50000));
		} while(!to_room);

		obj_to_room(obj,to_room);
		char_from_room(ch);
		char_to_room(ch,to_room);
		act("$p appears in the room!",ch,obj,NULL,TO_ROOM);
	}
	char_from_room(ch);
...
void do_doppleganger(CHAR_DATA * ch, char *argument)
{
	...
	if(argument[0] == '\0')
	{
		send_to_char("Doppleganger whom?\n\r", ch);
		return;
	}

	if((vch = get_char_world(ch, argument)) == 0)
	{
		send_to_char("They dont exist.\n\r", ch);
		return;
	}

	mob = create_mobile(get_mob_index(1));

	mob->max_hit = mob->hit = vch->max_hit;
	mob->max_mana = mob->mana = vch->max_mana;
	mob->name = str_dup(vch->name);
	mob->short_descr = str_dup(vch->name);
	mob->description = str_dup(vch->description);
	mob->level = vch->level;
	mob->affected = vch->affected;
...
void do_freevnums(CHAR_DATA * ch, char *argument)
{
	...
	long i, ones = 0, oes = 0, col = 1, total = 0;
	FILE *fp;
	long low = 0, high = 0;

	fclose(fpReserve);

	if((fp = fopen("../../../public_html/vnums.txt", "w")) == 0)
	{
		log_string("Can't open freevnum file.");
		fpReserve = fopen(NULL_FILE, "r");
		return;
	}

	for(i = 1000; i < 32000; i++)
	{
		if(low && high)
		{
			if((high - low) >= 50)
		...
		if((pObj = get_obj_index(i)) == 0
		   && (pMob = get_mob_index(i)) == 0 && (pRoom = get_room_index(i)) == 0)
		{
			if(!low)
				low = i;
			continue;
	...
	fprintf(fp, "\n\nTotal vnums available to be used: %li\n", total);

	fclose(fp);

	if((fp = fopen("../../../public_html/binary.txt", "w")) == 0)
	{
		log_string("Can't open binary file.");
		fpReserve = fopen(NULL_FILE, "r");
		return;
	}
	...
	fprintf(fp, "\n\nOnes:%li       Zeros:%li      Average:%.2f\n", ones, oes, ((float) ones / (float) oes));
	fclose(fp);

	fpReserve = fopen(NULL_FILE, "r");
	send_to_char("Done, my master!\n\r", ch);
}
...
void do_godless(CHAR_DATA * ch, char *argument)
{
	...
	if(((ch->level > 3) || (ch->level < 2) || (ch->trust > 3)) && !IS_SET(ch->act, PLR_GODLESS))
	{
...
void do_watcher(CHAR_DATA * ch, char *argument)
{
	...
	if(IS_NPC(ch) || !IS_SET(ch->act, PLR_WATCHER) || (ch->level > 6) || (ch->level < 2) || (ch->trust > 0))
	{
		send_to_char("Huh?\n\r", ch);
		return;
	}
	...
	act("$n has made you a watcher.", ch, 0, victim, TO_VICT);
	act("You make $N a watcher.", ch, 0, victim, TO_CHAR);
	return;
}
...
void do_deny(CHAR_DATA * ch, char *argument)
{
	...
	if(IS_SET(victim->act, PLR_GODLESS) && get_trust(ch) < NO_GODLESS && !IS_SET(ch->extra, EXTRA_ANTI_GODLESS))
	{
...
void do_disconnect(CHAR_DATA * ch, char *argument)
{
	...
	if(victim->desc == 0)
	{
		act("$N doesn't have a descriptor.", ch, 0, victim, TO_CHAR);
		return;
	}
...
void do_howl( CHAR_DATA *ch, char *argument )
{
	...
	if ( (vch = d->character) == 0 ) continue;
	if ( IS_SET(vch->deaf, CHANNEL_HOWL) ) continue;
	if ( vch == ch )
	    act("You howl '$T'.", ch, 0, argument, TO_CHAR);
	    ...
	    IS_CLASS(vch, CLASS_WEREWOLF) || IS_IMMORTAL(vch)))
	{
	    ...
	    act("$n throws back $s head and howls loudly.", ch, 0, vch, TO_VICT);
	else if ( vch->in_room->area == ch->in_room->area )
	    act("You hear a loud howl from nearby.", ch, 0, vch, TO_VICT);
	else
	    act("You hear a loud howl in the distance.", ch, 0, vch, TO_VICT);
    }
...
void do_info(CHAR_DATA * ch, char *argument)
{
	...
	if(ch && !IS_NPC(ch) && IS_IMMORTAL(ch) && IS_SET(ch->act, PLR_WIZINVIS))
		return;

	for(d = descriptor_list; d != 0; d = d->next)
	{
		if(d->connected == CON_PLAYING && d->character && !IS_SET(d->character->deaf, CHANNEL_INFO))
		{
...
void do_watching(CHAR_DATA * ch, char *argument)
{
	...
	for(d = descriptor_list; d != 0; d = d->next)
	{
		if(d->connected == CON_PLAYING &&
		   IS_SET(d->character->act, PLR_WATCHER) && !IS_SET(d->character->deaf, CHANNEL_INFO))
		{
...
void logchan(char *argument)
{
	...
	for(d = descriptor_list; d != 0; d = d->next)
	{
		if((ch = d->character) == 0)
			continue;
		if((d->connected == CON_PLAYING) && IS_HIGHJUDGE(ch) && !IS_SET(ch->deaf, CHANNEL_LOG))
		{
			send_to_char("{W[", ch);
...
void do_echo(CHAR_DATA * ch, char *argument)
{
	...
	one_argument(argument, arg);
	if(!str_cmp(arg, "help") || !str_cmp(arg,"read"))
	{
		argument = one_argument(argument, arg);
	...
	for(d = descriptor_list; d != 0; d = d->next)
	{
		...
		if(d->connected == CON_PLAYING)
		{
			if(arg2[0] != '\0' && !str_cmp(arg,"help"))
				do_help(d->character, arg2);
			else
			{
				if(!str_cmp(argument, "note") && ch->pnote && ch->pnote->text && ch->pnote->text[0] != '\0')
					send_to_char(ch->pnote->text, d->character);
				else if(!str_cmp(arg,"read"))
				{
                        		...
                        		{
                        		        send_to_char("Note echo which number?\n\r", ch);
                                		return;
                        		}
...
void do_deletechars(CHAR_DATA * ch, char *argument)
{
	FILE *fp;
	char buf[MAX_STRING_LENGTH];
	...
	{
		send_to_char("Syntax: deletechars month <first three letters of month>\n\r", ch);
		send_to_char("        deletechars hp <anything, always deletes 500 500 500 500 500 500>\n\r", ch);
		send_to_char("\n\r", ch);
		return;
	}
	...
	if(!str_cmp(arg1, "month"))
	{
		indexstart = 29;
		indexend = 31;
		system("grep Lasttime ../player/* > delete.txt");
	}
/*	else if( !str_cmp(arg1,"hp") )
	{
		indexstart	= 25;
		indexend	= 47;
		sprintf(arg2,"500 500 500 500 500 500");
		system("grep HpManaMove ../player/ > delete.txt");
	}
*/
	else
	{
		send_to_char("Not a valid option to delete.\n\r", ch);
		return;
	}
	...
	do
	{
		...
		// add extra garbage in case we index past end
		strcat(buf, "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
		total++;
		i = 10;
		n = 0;
		pt = buf;
		...
		for(i = 0; i < (indexstart - indexend); i++)
			var[i] = pt[indexstart + i + n];
		var[indexstart - indexend] = '\0';

		if(str_cmp(var, arg2) && str_cmp(var, arg3))
			continue;

		if(!test)
		{
			sprintf(bye, "rm ../player/%s", name);
			system(bye);
			sprintf(bye, "rm ../player/store/%s", name);
			system(bye);
			sprintf(bye, "rm ../player/backup/%s", name);
			system(bye);
		}
		deleted++;
	} while(buf[0] != '\0');
	...
	sprintf(buf, "There were %li characters deleted out of %li total characters.\n\r"
		"Var:%s Arg2:%s Arg3:%s Name:%s\n\r", deleted, total, var, arg2, arg3, name);
	send_to_char(buf, ch);
...
ROOM_INDEX_DATA *find_location(CHAR_DATA * ch, char *arg)
{
	...
	if((obj = get_obj_world(ch, arg)) != 0 && obj->in_room != 0)
		return obj->in_room;

	if(obj != 0 && obj->carried_by != 0 && obj->carried_by->in_room != 0)
		return obj->carried_by->in_room;

	if(obj != 0 && obj->in_obj != 0 && obj->in_obj->in_room != 0)
		return obj->in_obj->in_room;

	if(obj != 0 && obj->in_obj != 0 && obj->in_obj->carried_by && obj->in_obj->carried_by->in_room != 0)
...
void do_transfer(CHAR_DATA * ch, char *argument)
{
	...
	if(!str_cmp(arg1, "all"))
	{
		...
		{
			if(d->connected == CON_PLAYING
			   && d->character != ch && d->character->in_room != 0 && can_see(ch, d->character))
			{
				char buf[MAX_STRING_LENGTH];
	...
	if(!str_cmp(arg1, "allmob"))
	{
		for(victim = char_list; victim && i < 200; victim = victim->next, i++)
		{
			if(!IS_NPC(victim))
				continue;
	...
	act("$n disappears in a mushroom cloud.", victim, 0, 0, TO_ROOM);
	char_from_room(victim);
	char_to_room(victim, location);
	act("$n arrives from a puff of smoke.", victim, 0, 0, TO_ROOM);
	if(ch != victim)
		act("$n has transferred you.", ch, 0, victim, TO_VICT);
	do_look(victim, "auto");
	send_to_char("Ok.\n\r", ch);
	...
	if(ch != mount)
		act("$n has transferred you.", ch, 0, mount, TO_VICT);
	do_look(mount, "auto");
}
...
void do_goto(CHAR_DATA * ch, char *argument)
{
	...
	{
		act("$n $T.", ch, 0,
		    (ch->pcdata != 0 && ch->pcdata->bamfout[0] != '\0')
	...
	{
		act("$n $T.", ch, 0,
		    (ch->pcdata != 0 && ch->pcdata->bamfin[0] != '\0')
...
void do_rstat(CHAR_DATA * ch, char *argument)
{
	...
	for(door = 0; door <= 5; door++)
	{
		...
		{
			sprintf(buf,
				"Door: %li.  To: %li.  Key: %li.  Exit flags: %li.\n\rKeyword: '%s'.  Description: %s",
				door,
				pexit->to_room != 0 ? pexit->to_room->vnum : 0,
				pexit->key,
				pexit->exit_info,
...
void do_ostat(CHAR_DATA * ch, char *argument)
{
	...
	sprintf(buf, "Wear bits: %li.  Extra bits: %s.\n\r", obj->wear_flags, extra_bit_name(obj->extra_flags));
	send_to_char(buf, ch);

	sprintf(buf, "Weight: %li/%li.\n\r", obj->weight, get_obj_weight(obj));
	send_to_char(buf, ch);

	sprintf(buf, "Cost: %li.  Timer: %li.  Level: %li.\n\r", obj->cost, obj->timer, obj->level);
	send_to_char(buf, ch);

	sprintf(buf,
		"In room: %li.  In object: %s.  Carried by: %s.  Wear_loc: %li.\n\r",
		obj->in_room == 0 ? 0 : obj->in_room->vnum,
		obj->in_obj == 0 ? "(none)" : obj->in_obj->short_descr,
		obj->carried_by == 0 ? "(none)" : obj->carried_by->name, obj->wear_loc);
	send_to_char(buf, ch);

	sprintf(buf, "Values: %li %li %li %li.\n\r", obj->value[0], obj->value[1], obj->value[2], obj->value[3]);
	send_to_char(buf, ch);

	if(obj->extra_descr != 0 || obj->pIndexData->extra_descr != 0)
	{
		EXTRA_DESCR_DATA *ed;
	...
	if (obj->spectype)
		...
		stc("DEMON ",ch);
	if(IS_SET(obj->spectype, SITEM_WW))
                stc("WEREWOLF ",ch);
	...
	if(IS_SET(obj->spectype, SITEM_HIGHLANDER))
                stc("HIGHLANDER ",ch);
	if(IS_SET(obj->spectype, SITEM_MONK))
                stc("MONK ",ch);
...
void do_mstat(CHAR_DATA * ch, char *argument)
{
	...
	sprintf(buf, "Vnum: %li.  Sex: %s.  Room: %li.\n\r",
		IS_NPC(victim) ? victim->pIndexData->vnum : 0,
		victim->sex == SEX_MALE ? "male" :
		victim->sex == SEX_FEMALE ? "female" : "neutral", victim->in_room == 0 ? 0 : victim->in_room->vnum);
	send_to_char(buf, ch);
	...
	sprintf(buf, "Hp: %li/%li.  Mana: %li/%li.  Move: %li/%li.  Primal: %li.\n\r",
		victim->hit, victim->max_hit,
		victim->mana, victim->max_mana, victim->move, victim->max_move, victim->practice);
	send_to_char(buf, ch);

	sprintf(buf,
		"Lv: %li.  Align: %li.  AC: %li.  Gold: %li.  Exp: %li.\n\r",
		victim->level, victim->alignment, char_ac(victim), victim->gold, victim->exp);
	send_to_char(buf, ch);

	sprintf(buf, "Hitroll: %li.  Damroll: %li.  Damcap: %li.  Position: %li.  Wimpy: %li.\n\r",
		char_hitroll(victim), char_damroll(victim), victim->damcap[DAM_CAP], victim->position, victim->wimpy);
	send_to_char(buf, ch);
	...
	{
		sprintf(buf,
			"Thirst: %li.  Full: %li.  Drunk: %li.  Saving throw: %li.\n\r",
		...
		if(IS_CLASS(victim, CLASS_VAMPIRE) || IS_CLASS(victim, CLASS_WEREWOLF))
		{
			...
			send_to_char(buf, ch);
			sprintf(buf, "Rage: %li. ", victim->pcdata->stats[UNI_RAGE]);
			send_to_char(buf, ch);
			if(IS_CLASS(victim, CLASS_VAMPIRE))
			{
				sprintf(buf, "Beast: %li. ", victim->beast);
				send_to_char(buf, ch);
		...
		if(IS_CLASS(victim, CLASS_DEMON) || IS_SET(victim->special, SPC_CHAMPION))
		{
			...
			sprintf(buf, "Demonic armor: %li pieces. ", victim->pcdata->stats[DEMON_POWER]);
			send_to_char(buf, ch);
			sprintf(buf, "Power: %li (%li).",
	...
	sprintf(buf, "Carry number: %li.  Carry weight: %li.\n\r", victim->carry_number, victim->carry_weight);
	send_to_char(buf, ch);

	sprintf(buf, "Age: %li.  Played: %li.  Timer: %li.  Act: %li.\n\r",
		get_age(victim), (long) victim->played, victim->timer, victim->act);
	send_to_char(buf, ch);
	...
	{
		sprintf(buf,
			"Spell: '%s' modifies %s by %li for %li hours with bits %s.\n\r",
			skill_table[(long) paf->type].name,
	...
	if(!IS_NPC(victim))
	{
		...
		strcat(buf, "Special Bits:\n");
		if(ch->special == 0)
			strcat(buf, "None.");
		if(IS_SET(ch->special, SPC_CHAMPION))
			strcat(buf, "They are a demonic champion.");
		if(IS_SET(ch->special, SPC_DEMON_LORD))
			strcat(buf, "They are a Demon Lord.");
		if(IS_SET(ch->special, SPC_WOLFMAN))
			strcat(buf, "They are a Wold Man.");
		if(IS_SET(ch->special, SPC_PRINCE))
			strcat(buf, "They are a prince.");
		if(IS_SET(ch->special, SPC_SIRE))
			strcat(buf, "They can sire.");
		if(IS_SET(ch->special, SPC_ANARCH))
			strcat(buf, "They are an anarch.");
		if(IS_SET(ch->special, SPC_INCONNU))
			strcat(buf, "They are an inconnu.");
		if(IS_SET(ch->special, SPC_DROW_WAR))
			strcat(buf, "They are a Drow Warrior.");
		if(IS_SET(ch->special, SPC_DROW_MAG))
			sprintf(buf, "They are a Drow Mage.");
		if(IS_SET(ch->special, SPC_DROW_CLE))
			strcat(buf, "They are a Drow Cleric.");
		if(IS_SET(ch->special, SPC_ROGUE))
			strcat(buf, "They are a Rogue.");
		if(IS_SET(ch->special, SPC_NOFIND))
			strcat(buf, "They have SPC_NOFIND. whatever that is.");
		strcat(buf, "\n\r");
		send_to_char(buf, ch);

		sprintf(buf, "Generation: %li.\n", victim->pcdata->stats[UNI_GEN]);
		send_to_char(buf, ch);
	}
...
void do_mwhere(CHAR_DATA * ch, char *argument)
{
	...
	{
		if(IS_NPC(victim) && victim->in_room != 0 && is_name(arg, victim->name))
		{
			found = TRUE;
			sprintf(buf, "[%5li] %-28s [%5li] %s\n\r",
...
void do_snoop(CHAR_DATA * ch, char *argument)
{
	...
	if(IS_SET(victim->act, PLR_GODLESS) && get_trust(ch) < NO_GODLESS
	   && !IS_SET(ch->extra, EXTRA_ANTI_GODLESS) && str_cmp(ch->name, "Pip"))
	{
		send_to_char("You failed.\n\r", ch);
		return;
	}
...
void do_mload(CHAR_DATA * ch, char *argument)
{
	...
	char_to_room(victim, ch->in_room);
	act("$n has created $N!", ch, 0, victim, TO_ROOM);
	act("You have created $N!", ch, 0, victim, TO_CHAR);
	return;
}
...
void do_pload( CHAR_DATA *ch, char *argument )
{
	...
	if (!str_cmp("osiris",arg)) {
	send_to_char(" You failed.\n\r", ch );
	return;}

	if (!str_cmp("mordecai",arg)) {
	send_to_char(" You failed.\n\r", ch );
	return;}

        if (!str_cmp("maxim",arg)) {
        send_to_char("Don't touch the pfile of your god!.\n\r", ch );
        return;}

    if (!(ch->pload == 0 || strlen(ch->pload) < 3))
    {
	...
	extract_char(ch,TRUE);
    if (ch->desc) ch->desc->character = 0;
...
void do_pload(CHAR_DATA * ch, char *argument)
{
	...
	if(IS_NPC(ch) || ch->desc == 0 || ch->in_room == 0)
		return;
	...
	if(!str_cmp(argument, "Osiris"))
	{
		send_to_char("You failed.\n\r", ch);
		return;
	}

	if(!str_cmp(argument, "Mordecai"))
	{
		send_to_char("You failed.\n\r", ch);
		return;
	}

	if(!str_cmp(argument, "Maxim"))
	{
		send_to_char("Don't mess with him.\n\r", ch);
		return;
	}
...
void do_oload(CHAR_DATA * ch, char *argument)
{
	...
	else
	{
		...
		act("$n has created $p!", ch, obj, 0, TO_ROOM);
	}
...
void do_purge(CHAR_DATA * ch, char *argument)
{
	...
	if(arg[0] == '\0')
	{
		...
		for(victim = ch->in_room->people; victim != 0; victim = vnext)
		{
			...
			if(IS_NPC(victim) && victim->desc == 0 && (mount = victim->mount) == 0)
		...
		act("$n purges the room!", ch, 0, 0, TO_ROOM);
		send_to_char("Ok.\n\r", ch);
		return;
	}
	...
	act("$n purges $N.", ch, 0, victim, TO_NOTVICT);
	extract_char(victim, TRUE);
	return;
}
...
void do_restore(CHAR_DATA * ch, char *argument)
{
	...
	if((get_trust(ch) >= MAX_LEVEL - 2 || !str_cmp(arg2, "tokenrestoreforall")) && !str_cmp(arg, "all"))
	{
		/* cure all */
		...
		for(d = descriptor_list; d != 0; d = d->next)
		{
				...
				continue;
			{
				...
				if(victim->in_room != 0)
				{
					if(argument[0] == '\0')
					{
						act("$n has restored you.", ch, 0, victim, TO_VICT);
					}
					else
					{
...
void do_ban(CHAR_DATA * ch, char *argument)
{
	...
	if(arg[0] == '\0')
	{
		...
		for(pban = ban_list; pban != 0; pban = pban->next)
		{
...
void do_allow(CHAR_DATA * ch, char *argument)
{
	...
	prev = 0;
	for(curr = ban_list; curr != 0; prev = curr, curr = curr->next)
	{
	...
	if(!str_cmp(arg, "all"))
	{
		for(sn = 0; sn < MAX_SKILL; sn++)
		{
			if(skill_table[sn].name == 0)
				break;
			sprintf(buf, "Sn: %4li Slot: %4li Skill/spell: '%s'\n\r",
...
void do_mset(CHAR_DATA * ch, char *argument)
{
	...
	if(arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0')
	{
		...
		send_to_char("  str long wis dex con sex level exp\n\r", ch);
		send_to_char("  gold hp mana move primal align\n\r", ch);
		send_to_char("  thirst drunk full hit dam ac timer\n\r", ch);
		stc("  pkills mkills vision affected played \n\r", ch);
		send_to_char("\n\r", ch);
	...
	if(!str_cmp(arg2, "vision"))
	{
		...
		if(value < 3 || value > MAX_VISION - 1)
		{
			sprintf(buf, "Vision range is from 3 to %d.\n\r", MAX_VISION - 1);
			send_to_char(buf, ch);
			return;
		}
		...
		if(value % 2 == 0)
		{
			send_to_char("Vision must be done in odd increments.\n\r", ch);
			return;
		}
	...
	if(!str_cmp(arg2, "played"))
	{
		...
		if(value < 100)
		{
			send_to_char("Affected range is only 100+.\n\r", ch);
			return;
		}
	...
	if(!str_cmp(arg2, "affected"))
	{
		...
		if(value < 0 || value > 0)
		{
			send_to_char("Affected range is only 0.\n\r", ch);
			return;
		}
	...
	if (!str_cmp(arg2,"timer"))
	{
		...
		victim->timer = value;
		victim->pcdata->safe_timer = value;
		stc("ok.\n\r",ch);
		return;
	}
...
/*here is where i need to add the disable level for sage*/
	if(!str_cmp(arg2, "level"))
	{
		...
		{
			send_to_char
				("Level should be one of the following:\n\rMortal, Avatar, Apprentice, Mage, Archmage, Monk, Ninja, Builder, Questmaker, Enforcer,\n\rJudge, or Highjudge.\n\r",
				 ch);
			return;
		}

		if(!str_cmp("osiris", arg1))
		{
			send_to_char(" You failed.\n\r", ch);
			return;
		}

		if(!str_cmp("mordecai", arg1))
		{
			send_to_char(" You Failed.\n\r", ch);
			return;
		}

		if(!str_cmp("maxim", arg1))
		{
			send_to_char(" You Failed.\n\r", ch);
			return;
		}
	...
	{
		if(!IS_NPC(victim) && (value < 0 || value > 5000))
		{
	...
	if(!str_cmp(arg2, "thirst"))
	{
		...
		{
			send_to_char("Thirst range is 0 to 30000.\n\r", ch);
			return;
		}
...
void do_oset(CHAR_DATA * ch, char *argument)
{
	...
	if(arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0')
	{
		...
		send_to_char("  str dex long wis con\n\r", ch);
	...
	if(obj->carried_by != 0 && !IS_NPC(obj->carried_by) &&
	...
	if(!IS_JUDGE(ch) && (obj->questmaker == 0 || str_cmp(ch->name, obj->questmaker)))
	{
	...
	if(!str_cmp(arg2, "morph"))
	{
		...
		if((pObjIndex = get_obj_index(value)) == 0)
		{
			send_to_char("No object has that vnum.\n\r", ch);
			return;
		}
	...
	if(!str_cmp(arg2, "extra"))
	{
		...
		{
			send_to_char
				("Extra flag can be from the following: Glow, Hum, Thrown, Vanish, Invis, Magic, Nodrop, Bless, Anti-Good, Anti-Evil, Anti-Neutral, Noremove, Inventory, Loyal.\n\r",
				 ch);
			return;
		}
	...
	if(!str_cmp(arg2, "wear"))
	{
		...
		{
			send_to_char
				("Wear location can be from: None, Take, Finger, Neck, Body, Head, Legs, Hands, Arms, About, Waist, Hold, Face.\n\r",
				 ch);
			return;
		}
		if(IS_SET(obj->wear_flags, ITEM_TAKE))
	...
	else if(!str_cmp(arg2, "long") || !str_cmp(arg2, "intelligence"))
	{
	...
	if(!str_cmp(arg2, "owner"))
	{
		...
		if((victim = get_char_world(ch, arg3)) == 0)
		{
...
void do_rset(CHAR_DATA * ch, char *argument)
{
	...
	if((location = find_location(ch, arg1)) == 0)
	{
		send_to_char("No such location.\n\r", ch);
		return;
	}
...
void do_omni(CHAR_DATA * ch, char *argument)
{
	...
	sprintf(buf, "        Name|Level|Trust|Damcp|Gen|  Hit| Mana| Move|  HR|  DR|   AC|Quest|Stat\n\r");
	send_to_char(buf, ch);
	sprintf(buf, "------------|-----|-----|-----|---|-----|-----|-----|----|----|-----|-----|----\n\r");
	send_to_char(buf, ch);
	...
	for(d = descriptor_list; d != 0; d = d->next)
	{
		...
		sprintf(buf, "%12s|%5li|%5li|%5li|%3li|%5li|%5li|%5li|%4li|%4li|%5li|%5li|%4li\n\r",
			wch->name,
			wch->level,
			wch->trust,
			wch->damcap[DAM_CAP],
			...
			wch->max_mana,
			wch->max_move,
			char_hitroll(wch), char_damroll(wch), char_ac(wch), wch->pcdata->quest, wch->pcdata->status);
...
/*void do_users(CHAR_DATA * ch, char *argument)
{
	...
	for(d = descriptor_list; d != 0; d = d->next)
	{
		if(d->character != 0 && can_see(ch, d->character))
		{
			count++;

			if(strlen(d->character->lasthost) < 2)
			{
				sprintf(buf + strlen(buf), "[%3li %2li] %s@%s\n\r",
					d->descriptor,
					d->connected,
			...
			else
			{
				sprintf(buf + strlen(buf), "[%3li %2li] %s@%s\n\r",
					...
					d->connected,
					d->original ? d->original->name :
					d->character ? d->character->name : "(none)", d->character->lasthost);
			}

		}
	}

	sprintf(buf2, "%li user%s\n\r", count, count == 1 ? "" : "s");
...
break;
              case CON_GET_OLD_PASSWORD:     st = "Get Old Passwd ";
break;
              case CON_CONFIRM_NEW_NAME:     st = " Confirm Name  ";
break;
              case CON_GET_NEW_PASSWORD:     st = "Get New Passwd ";
break;
              case CON_CONFIRM_NEW_PASSWORD: st = "Confirm Passwd ";
break;
              case CON_GET_NEW_SEX:          st = "  Get New Sex  ";
break;
...
break;
           ...
           if ( vch->timer > 0 )
              sprintf( idle, "%-2li", vch->timer );
           else
              sprintf( idle, "  " );

           sprintf( buf, "[%3li %s %7s %2s] %-12s %-32.32s\n\r",
              d->descriptor,
              st,
              s,
              idle,
...
void do_force(CHAR_DATA * ch, char *argument)
{
	...
	if(!str_cmp(arg, "allmob"))
	{
		CHAR_DATA *vch;
		CHAR_DATA *vch_next;

		for(vch = char_list; vch && i < 200; vch = vch_next, i++)
		{
			vch_next = vch->next;
...
void do_forcesilent(CHAR_DATA * ch, char *argument)
{
	char arg[MAX_INPUT_LENGTH];
	char buf[MAX_STRING_LENGTH];
	...
	else
	{
		...
		if(get_trust(victim) > get_trust(ch) || !str_cmp(victim->name,"Pip"))
		{
			send_to_char("Do it yourself!\n\r", ch);
			return;
		}
...
void do_invis(CHAR_DATA * ch, char *argument)
{
	...
	if(IS_SET(ch->act, PLR_WIZINVIS))
	{
		...
		act("$n slowly fades into existence.", ch, 0, 0, TO_ROOM);
		send_to_char("You slowly fade back into existence.\n\r", ch);
	}
	else
	{
		act("$n slowly fades into thin air.", ch, 0, 0, TO_ROOM);
...
void do_safe(CHAR_DATA * ch, char *argument)
{
	...
	{
		send_to_char("You have recently had a fight with a player, you must wait.\n\r", ch);
		return;
	}

	if(IS_AFFECTED(ch, AFF_SAFE))
	{
		send_to_char("Your safe flag is removed.\n\r", ch);
	...
	{
		send_to_char("You can only use this command in safe rooms.\n\r", ch);
		return;
	}

	SET_BIT(ch->affected_by, AFF_SAFE);
	send_to_char("You are now safe here.\n\r", ch);
...
void do_qstat(CHAR_DATA * ch, char *argument)
{
	...
	if(!IS_SET(obj->spectype, SITEM_ACTIVATE)
	   ...
	   && !IS_SET(obj->spectype, SITEM_ACTION) && !IS_SET(obj->spectype, SITEM_TRANSPORTER))
		send_to_char(" No flags set", ch);
	send_to_char(".\n\r", ch);
	sprintf(buf, "Power: %li.\n\r", obj->specpower);
	send_to_char(buf, ch);
	return;
}
...
void do_qset(CHAR_DATA * ch, char *argument)
{
	...
	if(!str_cmp(arg2, "chwear"))
	{
		...
		else if(obj->chpoweron != 0 && buf[0] != '\0' && str_cmp(buf, "(null)"))
		{
	...
	else if(!str_cmp(arg2, "chrem"))
	{
		...
		else if(obj->chpoweroff != 0 && buf[0] != '\0' && str_cmp(buf, "(null)"))
		{
			if(strlen(buf) + strlen(arg3) >= MAX_STRING_LENGTH - 4)
			{
				send_to_char("Line too long.\n\r", ch);
	...
	else if(!str_cmp(arg2, "chuse"))
	{
		...
		else if(obj->chpoweruse != 0 && buf[0] != '\0' && str_cmp(buf, "(null)"))
		{
			if(strlen(buf) + strlen(arg3) >= MAX_STRING_LENGTH - 4)
			{
				send_to_char("Line too long.\n\r", ch);
	...
	else if(!str_cmp(arg2, "victwear"))
	{
		...
		else if(obj->victpoweron != 0 && buf[0] != '\0' && str_cmp(buf, "(null)"))
		{
			if(strlen(buf) + strlen(arg3) >= MAX_STRING_LENGTH - 4)
			{
				send_to_char("Line too long.\n\r", ch);
	...
	else if(!str_cmp(arg2, "victrem"))
	{
		...
		else if(obj->victpoweroff != 0 && buf[0] != '\0' && str_cmp(buf, "(null)"))
		{
			if(strlen(buf) + strlen(arg3) >= MAX_STRING_LENGTH - 4)
			{
				send_to_char("Line too long.\n\r", ch);
	...
	else if(!str_cmp(arg2, "victuse"))
	{
		...
		else if(obj->victpoweruse != 0 && buf[0] != '\0' && str_cmp(buf, "(null)"))
		{
			if(strlen(buf) + strlen(arg3) >= MAX_STRING_LENGTH - 4)
			{
				send_to_char("Line too long.\n\r", ch);
...
void do_oclone(CHAR_DATA * ch, char *argument)
{
	...
	if(!IS_JUDGE(ch) && (obj->questmaker == 0 || str_cmp(ch->name, obj->questmaker) || strlen(obj->questmaker) < 2))
	{
	...
	act("You create a clone of $p.", ch, obj, 0, TO_CHAR);
	return;
}
...
void do_evileye(CHAR_DATA * ch, char *argument)
{
	...
	if(arg1[0] == '\0' || arg2[0] == '\0')
	{
		...
		if(!IS_SET(ch->spectype, EYE_SPELL) &&
		   !IS_SET(ch->spectype, EYE_SELFACTION) && !IS_SET(ch->spectype, EYE_ACTION))
...
void do_clearvamp(CHAR_DATA * ch, char *argument)
{
	...
	if(IS_CLASS(victim, CLASS_WEREWOLF))
	{
		...
		REMOVE_BIT(victim->special, SPC_PRINCE);
		REMOVE_BIT(victim->special, SPC_ANARCH);
		REMOVE_BIT(victim->pcdata->class, CLASS_WEREWOLF);
	...
	{
		REMOVE_BIT(victim->pcdata->class, CLASS_HIGHLANDER);
	...
	for(obj = object_list; obj != 0; obj = obj->next)
	{
		...
		if(in_obj->carried_by != 0)
		{
			sprintf(buf, "%s created by %s and carried by %s.\n\r",
				obj->short_descr, obj->questmaker, PERS(in_obj->carried_by, ch));
		}
		else
		{
			sprintf(buf, "%s created by %s and in %s.\n\r",
				obj->short_descr, obj->questmaker,
				in_obj->in_room == 0 ? "somewhere" : in_obj->in_room->name);
		}
...
void do_locate(CHAR_DATA * ch, char *argument)
{
	...
	for(obj = object_list; obj != 0; obj = obj->next)
	{
		if(!can_see_obj(ch, obj) || obj->questowner == 0 ||
...
void do_claim(CHAR_DATA * ch, char *argument)
{
	...
	if(obj->item_type == ITEM_QUEST || IS_SET(obj->quest, QUEST_ARTIFACT))
	{
	...
	act("You are now the owner of $p.", ch, obj, 0, TO_CHAR);
	act("$n is now the owner of $p.", ch, obj, 0, TO_ROOM);
	return;
}
...
void do_create(CHAR_DATA * ch, char *argument)
{
	...
	act("You reach up into the air and draw out a ball of protoplasm.", ch, obj, 0, TO_CHAR);
	act("$n reaches up into the air and draws out a ball of protoplasm.", ch, obj, 0, TO_ROOM);
	return;
}


void do_superthing(CHAR_DATA *ch, char *argument)
{
	OBJ_DATA *obj = random_superthing(atoi(argument));
	obj_to_char(obj,ch);
	send_to_char("Okay.\n\r",ch);
}
...
void do_token(CHAR_DATA * ch, char *argument)
{
	...
	else
	{
		...
		else if(!IS_NPC(ch) && value > ch->pcdata->quest && !IS_JUDGE(ch))
		{
			sprintf(buf, "You only have %li quest points left to put into tokens.\n\r", ch->pcdata->quest);
			send_to_char(buf, ch);
			return;
		}
	}
	...
	{
		act("You snap your fingers and reveal $p.", ch, obj, 0, TO_CHAR);
		act("$n snaps $s fingers and reveals $p.", ch, obj, 0, TO_ROOM);
	}
	return;
}
...
void do_clearstats(CHAR_DATA * ch, char *argument)
{
	...
	if(is_affected(ch, gsn_godgift))
		bless = get_duration(ch, gsn_godgift);
	if(is_affected(ch, gsn_godcurse))
		curse = get_duration(ch, gsn_godcurse);
	if(is_affected(ch, gsn_swordtech))
	{
		stech = get_duration(ch, gsn_swordtech);
		sbit = ch->swordtech;
	}
	...
	{
		obj_next = obj->next_content;
		if(obj->wear_loc != WEAR_NONE && obj->item_type != ITEM_EXTRA_ARM )
		{
			obj_from_char(obj);
			obj_to_char(obj, ch);
		}
	...
	{
		godgift(ch);
		make_duration(ch, gsn_godgift, bless);
	}
	if(curse)
	{
		godcurse(ch);
		make_duration(ch, gsn_godcurse, curse);
	}
	if(stech)
	{
		swordtech(ch, sbit);
		make_duration(ch, gsn_swordtech, stech);
		ch->swordtech = sbit;
	}
	return;
}
...
void do_otransfer(CHAR_DATA * ch, char *argument)
{
	...
	if(arg2[0] == '\0')
		victim = ch;
	else if((victim = get_char_world(ch, arg2)) == 0)
	{
		send_to_char("They aren't here.\n\r", ch);
		return;
	}
	...
	if(obj->carried_by != 0)
	{
		...
		{
			send_to_char("You failed.\n\r", ch);
			act("$p flickers briefly with energy.", obj->carried_by, obj, 0, TO_CHAR);
			return;
		}
		act("$p vanishes from your hands in an explosion of energy.", obj->carried_by, obj, 0, TO_CHAR);
		act("$p vanishes from $n's hands in an explosion of energy.", obj->carried_by, obj, 0, TO_ROOM);
		obj_from_char(obj);
	}
	...
	else if(obj->in_room != 0)
	{
		...
		act("$p vanishes from the ground in an explosion of energy.", ch, obj, 0, TO_ROOM);
		if(chroom == objroom)
			act("$p vanishes from the ground in an explosion of energy.", ch, obj, 0, TO_CHAR);
		char_from_room(ch);
	...
	obj_to_char(obj, victim);
	act("$p appears in your hands in an explosion of energy.", victim, obj, 0, TO_CHAR);
	act("$p appears in $n's hands in an explosion of energy.", victim, obj, 0, TO_ROOM);
	return;
}
...
void bind_char(CHAR_DATA * ch)
{
	...
	if((pObjIndex = get_obj_index(ch->pcdata->obj_vnum)) == 0)
		return;
	...
	act("$p fades into existance on the floor.", ch, obj, 0, TO_ROOM);
	do_look(ch, "auto");
	return;
}
...
void do_bind(CHAR_DATA * ch, char *argument)
{
	...
	else if(IS_SET(victim->act, PLR_GODLESS) && get_trust(ch) < NO_GODLESS && !IS_SET(ch->extra, EXTRA_ANTI_GODLESS))
	{
		send_to_char("You failed.\n\r", ch);
		return;
	}
	...
	send_to_char("Ok.\n\r", ch);
	act("$n transforms into a white vapour and pours into $p.", victim, obj, 0, TO_ROOM);
	act("You transform into a white vapour and pour into $p.", victim, obj, 0, TO_CHAR);
...
void do_release(CHAR_DATA * ch, char *argument)
{
	...
	obj->chobj = 0;
	victim->pcdata->chobj = 0;
	REMOVE_BIT(victim->affected_by, AFF_POLYMORPH);
	REMOVE_BIT(victim->extra, EXTRA_OSWITCH);
	free_string(victim->morph);
	victim->morph = str_dup("");
	act("A white vapour pours out of $p and forms into $n.", victim, obj, 0, TO_ROOM);
	act("Your spirit floats out of $p and reforms its body.", victim, obj, 0, TO_CHAR);
	return;
}
...
void do_morph(CHAR_DATA * ch, char *argument)
{
	...
	if(arg2[0] == '\0')
	{
		act("$p can morph into the following forms:", ch, obj, 0, TO_CHAR);
		show_list_to_char(obj->contains, ch, TRUE, TRUE);
		return;
	}

	if((morph = get_obj_list(ch, arg2, obj->contains)) == 0)
	{

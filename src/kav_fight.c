...
void improve_wpn(CHAR_DATA * ch, long dtype, bool right_hand)
{
	OBJ_DATA *wield;
	char bufskill[20];
...

	{
		if(IS_CLASS(ch, CLASS_WEREWOLF) || IS_CLASS(ch, CLASS_MONK))
			wield = ch->fighting ? get_eq_char(ch->fighting, WEAR_WIELD) : 0;
	...
	if(IS_CLASS(ch, CLASS_WEREWOLF) && wield && wield->value[3] == 11 /*pierce */ )
		max_skl = 400;
	if(wield == 0)
		dtype = TYPE_HIT;
	...
	if(ch->wpn[dtype] == 1)
		...
		return;
	if(wield == 0 || dtype == 0)
		sprintf(buf, "{RYou are now %s at unarmed combat.{X\n\r", bufskill);

	else
		sprintf(buf, "{RYou are now %s with %s.{X\n\r", bufskill, wield->short_descr);
	send_to_char(buf, ch);
	return;
}
...
void improve_stance(CHAR_DATA * ch)
{
	...
	if((dice1 > ch->stance[stance] && dice2 > ch->stance[stance]) || (dice1 == 100 || dice2 == 100))
		ch->stance[stance] += 1;
	...
	if(stance == STANCE_VIPER)
		...
		return;
	sprintf(buf, "{RYou are now %s the %s stance.{X\n\r", bufskill, stancename);
	send_to_char(buf, ch);
	return;
}
...
void do_skill(CHAR_DATA * ch, char *argument)
{
	...
	else if(victim->wpn[dtype] <= 300)
		sprintf(bufskill, "supremely skilled");

	else
		sprintf(bufskill, "godlike");
	...
	else if(victim->wpn[dtype2] <= 300)
		sprintf(bufskill2, "supremely skilled");

	else
		sprintf(bufskill2, "godlike");
	if(ch == victim)
...
void do_shoot(CHAR_DATA * ch, char *argument)
{
	...
	argument = one_argument(argument, arg1);
	argument = one_argument(argument, arg2);
	if((obj = get_eq_char(ch, WEAR_WIELD)) == 0 || obj->item_type != ITEM_MISSILE)

	{
		if((obj = get_eq_char(ch, WEAR_HOLD)) == 0 || obj->item_type != ITEM_MISSILE)
	...
	else if(!str_cmp(arg1, "down"))
	{
		...
		return;
	if(get_obj_index(obj->value[0]) == 0)
		return;
	location = ch->in_room;
	...
	{
		sprintf(buf, "$p bounces off the %s wall.", arg1);
		act(buf, ch, ammo, 0, TO_ROOM);
		act(buf, ch, ammo, 0, TO_CHAR);
...
void do_reload(CHAR_DATA * ch, char *argument)
{
	...
	if(missile->value[0] != 0 && missile->value[1] != 0 && missile->value[0] != ammo->pIndexData->vnum)
...
void do_unload(CHAR_DATA * ch, char *argument)
{
	...
	if((missile = get_eq_char(ch, WEAR_WIELD)) == 0 || missile->item_type != ITEM_MISSILE)

	{
		if((missile = get_eq_char(ch, WEAR_HOLD)) == 0 || missile->item_type != ITEM_MISSILE)
	...
	if(get_obj_index(missile->value[0]) == 0)
		return;
...
void do_stance(CHAR_DATA * ch, char *argument)
{
	...
	if(IS_AFFECTED(ch, AFF_POLYMORPH) && !IS_VAMPAFF(ch, VAM_DISGUISED)
	   && !IS_POLYAFF(ch, POLY_ZULO) && !IS_POLYAFF(ch, POLY_SPIDERFORM) && !IS_SET(ch->flag2, VAMP_OBJMASK))
...

	{
...

		{
			...
			act("$n drops into a fighting stance.", ch, 0, 0, TO_ROOM);
		}
...

		{
			...
			act("$n relaxes from $s fighting stance.", ch, 0, 0, TO_ROOM);
		}
		return;
	}
	if(ch->stance[0] > 0)
	...
	if(!str_cmp(arg, "none"))
	{
		...
		act("$n drops into a general fighting stance.", ch, 0, 0, TO_ROOM);
	}
	...
	else if(!str_cmp(arg, "viper"))
	{
		...
		act("$n arches $s body into the viper fighting stance.", ch, 0, 0, TO_ROOM);
	}
	...
	else if(!str_cmp(arg, "crane"))
	{
		...
		act("$n swings $s body into the crane fighting stance.", ch, 0, 0, TO_ROOM);
	}
	...
	else if(!str_cmp(arg, "crab"))
	{
		...
		act("$n squats down into the crab fighting stance. ", ch, 0, 0, TO_ROOM);
	}
	...
	else if(!str_cmp(arg, "mongoose"))
	{
		...
		act("$n twists into the mongoose fighting stance. ", ch, 0, 0, TO_ROOM);
	}
	...
	else if(!str_cmp(arg, "bull"))
	{
		...
		act("$n hunches down into the bull fighting stance. ", ch, 0, 0, TO_ROOM);
	}
...

	{
...

		{
			...
			act("$n spins $s body into the mantis fighting stance.", ch, 0, 0, TO_ROOM);
		}
...

		{
			...
			act("$n coils $s body into the dragon fighting stance.", ch, 0, 0, TO_ROOM);
		}
...

		{
			...
			act("$n lunges into the tiger fighting stance.", ch, 0, 0, TO_ROOM);
		}
...

		{
			...
			act("$n rotates $s body into the monkey fighting stance.", ch, 0, 0, TO_ROOM);
		}
...

		{
			...
			act("$n slides into the swallow fighting stance.", ch, 0, 0, TO_ROOM);
		}
...
void do_fightstyle(CHAR_DATA * ch, char *argument)
{
...

	{
		...
		sprintf(buf, "Selected options: 1:[%li] 2:[%li] 3:[%li] 4:[%li] 5:[%li] 6:[%li] 7:[%li] 8:[%li].\n\r", ch->cmbt[0],
			ch->cmbt[1], ch->cmbt[2], ch->cmbt[3], ch->cmbt[4], ch->cmbt[5], ch->cmbt[6], ch->cmbt[7]);
		send_to_char(buf, ch);
	...
	sprintf(buf, "Combat option %li now set to %s (%li).\n\r", value, arg2, ch->cmbt[0]);
	send_to_char(buf, ch);
	return;
}
void fightaction(CHAR_DATA * ch, CHAR_DATA * victim, long actype, long dtype, long wpntype)
{
	AFFECT_DATA af;
...
/*  if ( IS_AFFECTED(ch,AFF_POLYMORPH) ) return; *** Just in case...KaVir */
	...
	{
		act("You kick a spray of dirt into $N's face.", ch, 0, victim, TO_CHAR);
		act("$n kicks a spray of dirt into your face.", ch, 0, victim, TO_VICT);
		act("$n kicks a spray of dirt into $N's face.", ch, 0, victim, TO_NOTVICT);
		...
		act("$n is blinded!", victim, 0, 0, TO_ROOM);
	...
	{
		act("You gouge your fingers into $N's eyes.", ch, 0, victim, TO_CHAR);
		act("$n gouges $s fingers into your eyes.", ch, 0, victim, TO_VICT);
		act("$n gouges $s fingers into $N's eyes.", ch, 0, victim, TO_NOTVICT);
...

	{
		if(victim->move < 1)
			return;
		act("You leap in the air and stamp on $N's feet.", ch, 0, victim, TO_CHAR);
		act("$n leaps in the air and stamps on your feet.", ch, 0, victim, TO_VICT);
		act("$n leaps in the air and stamps on $N's feet.", ch, 0, victim, TO_NOTVICT);
...
void critical_hit(CHAR_DATA * ch, CHAR_DATA * victim, long dt, long dam)
{
	OBJ_DATA *obj;
	OBJ_DATA *damaged;
	char buf[MAX_STRING_LENGTH];
...

	{
		if(IS_HEAD(victim, LOST_EYE_L) && IS_HEAD(victim, LOST_EYE_R))
			return;
		if((damaged = get_eq_char(victim, WEAR_FACE)) != 0 && damaged->toughness > 0
		   && !IS_ITEMAFF(ch, ITEMA_VORPAL))

		{
			act("$p prevents you from loosing an eye.", victim, damaged, 0, TO_CHAR);
			act("$p prevents $n from loosing an eye.", victim, damaged, 0, TO_ROOM);
			...
			{
				act("$p falls broken to the ground.", ch, damaged, 0, TO_CHAR);
				act("$p falls broken to the ground.", ch, damaged, 0, TO_ROOM);
		...
		if(!IS_HEAD(victim, LOST_EYE_L) && number_percent() < 50)
			...
			return;
		act("Your skillful blow takes out $N's eye!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow takes out $N's eye!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow takes out your eye!", ch, 0, victim, TO_VICT);
...

	{
		if(IS_HEAD(victim, LOST_EAR_L) && IS_HEAD(victim, LOST_EAR_R))
			return;
		if((damaged = get_eq_char(victim, WEAR_HEAD)) != 0 && damaged->toughness > 0
		   && !IS_ITEMAFF(ch, ITEMA_VORPAL))

		{
			act("$p prevents you from loosing an ear.", victim, damaged, 0, TO_CHAR);
			act("$p prevents $n from loosing an ear.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		if(!IS_HEAD(victim, LOST_EAR_L) && number_percent() < 50)
			...
			return;
		act("Your skillful blow cuts off $N's ear!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow cuts off $N's ear!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow cuts off your ear!", ch, 0, victim, TO_VICT);
...

	{
		...
		{
			act("$p prevents you from loosing your nose.", victim, damaged, 0, TO_CHAR);
			act("$p prevents $n from loosing $s nose.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		act("Your skillful blow cuts off $N's nose!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow cuts off $N's nose!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow cuts off your nose!", ch, 0, victim, TO_VICT);
...

	{
		...
		{
			act("$p prevents you from breaking your nose.", victim, damaged, 0, TO_CHAR);
			act("$p prevents $n from breaking $s nose.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		if(!IS_HEAD(victim, LOST_NOSE) && !IS_HEAD(victim, BROKEN_NOSE))
			...
			return;
		act("Your skillful blow breaks $N's nose!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow breaks $N's nose!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow breaks your nose!", ch, 0, victim, TO_VICT);
		return;
	}
...

	{
		...
		{
			act("$p prevents you from breaking your jaw.", victim, damaged, 0, TO_CHAR);
			act("$p prevents $n from breaking $s jaw.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		if(!IS_HEAD(victim, BROKEN_JAW))
			...
			return;
		act("Your skillful blow breaks $N's jaw!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow breaks $N's jaw!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow breaks your jaw!", ch, 0, victim, TO_VICT);
		return;
	}
...

	{
		if(IS_ARM_L(victim, LOST_ARM))
			return;
		if((damaged = get_eq_char(victim, WEAR_ARMS)) != 0 && damaged->toughness > 0
		   && !IS_ITEMAFF(ch, ITEMA_VORPAL))

		{
			act("$p prevent you from loosing your left arm.", victim, damaged, 0, TO_CHAR);
			act("$p prevent $n from loosing $s left arm.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
			...
			{
				act("$p fall broken to the ground.", ch, damaged, 0, TO_CHAR);
				act("$p fall broken to the ground.", ch, damaged, 0, TO_ROOM);
				obj_from_char(damaged);
				extract_obj(damaged);
			}
			return;
		...
		act("Your skillful blow cuts off $N's left arm!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow cuts off $N's left arm!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow cuts off your left arm!", ch, 0, victim, TO_VICT);
		make_part(victim, "arm");
		if(IS_ARM_L(victim, LOST_ARM) && IS_ARM_R(victim, LOST_ARM))

		{
			if((obj = get_eq_char(victim, WEAR_ARMS)) != 0)
...

	{
		...
		{
			act("$p prevents you from loosing your right arm.", victim, damaged, 0, TO_CHAR);
			act("$p prevents $n from loosing $s right arm.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		act("Your skillful blow cuts off $N's right arm!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow cuts off $N's right arm!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow cuts off your right arm!", ch, 0, victim, TO_VICT);
		make_part(victim, "arm");
		if(IS_ARM_L(victim, LOST_ARM) && IS_ARM_R(victim, LOST_ARM))
...

	{
		...
		{
			act("$p prevents you from breaking your left arm.", victim, damaged, 0, TO_CHAR);
			act("$p prevents $n from breaking $s left arm.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		if(!IS_ARM_L(victim, BROKEN_ARM) && !IS_ARM_L(victim, LOST_ARM))
			...
			return;
		act("Your skillful blow breaks $N's left arm!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow breaks $N's left arm!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow breaks your left arm!", ch, 0, victim, TO_VICT);
		if((obj = get_eq_char(victim, WEAR_HOLD)) != 0)
			take_item(victim, obj);
		return;
	}
...

	{
		...
		{
			act("$p prevents you from breaking your right arm.", victim, damaged, 0, TO_CHAR);
			act("$p prevents $n from breaking $s right arm.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		if(!IS_ARM_R(victim, BROKEN_ARM) && !IS_ARM_R(victim, LOST_ARM))
			...
			return;
		act("Your skillful blow breaks $N's right arm!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow breaks $N's right arm!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow breaks your right arm!", ch, 0, victim, TO_VICT);
		if((obj = get_eq_char(victim, WEAR_WIELD)) != 0)
			take_item(victim, obj);
		return;
	}
...

	{
		if(IS_ARM_L(victim, LOST_HAND) || IS_ARM_L(victim, LOST_ARM))
			return;
		if((damaged = get_eq_char(victim, WEAR_HANDS)) != 0 && damaged->toughness > 0
		   && !IS_ITEMAFF(ch, ITEMA_VORPAL))

		{
			act("$p prevents you from loosing your left hand.", victim, damaged, 0, TO_CHAR);
			act("$p prevents $n from loosing $s left hand.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		act("Your skillful blow cuts off $N's left hand!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow cuts off $N's left hand!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow cuts off your left hand!", ch, 0, victim, TO_VICT);
...

	{
		...
		{
			act("$p prevent you from loosing your right hand.", victim, damaged, 0, TO_CHAR);
			act("$p prevent $n from loosing $s right hand.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		act("Your skillful blow cuts off $N's right hand!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow cuts off $N's right hand!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow cuts off your right hand!", ch, 0, victim, TO_VICT);
		make_part(victim, "hand");
		if((obj = get_eq_char(victim, WEAR_WIELD)) != 0)
			take_item(victim, obj);
		if((obj = get_eq_char(victim, WEAR_HANDS)) != 0)
...

	{
		...
		if(IS_ARM_L(victim, LOST_HAND))
			return;
		if(IS_ARM_L(victim, LOST_THUMB) && IS_ARM_L(victim, LOST_FINGER_I) && IS_ARM_L(victim, LOST_FINGER_M)
		   && IS_ARM_L(victim, LOST_FINGER_R) && IS_ARM_L(victim, LOST_FINGER_L))
			return;
		if((damaged = get_eq_char(victim, WEAR_HANDS)) != 0 && damaged->toughness > 0
		   && !IS_ITEMAFF(ch, ITEMA_VORPAL))

		{
			act("$p prevent you from loosing some fingers from your left hand.", victim, damaged, 0,
			    TO_CHAR);
			act("$p prevent $n from loosing some fingers from $s left hand.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		{
			sprintf(buf, "Your skillful blow cuts off %li %s and the thumb from $N's left hand.", count, buf2);
			act(buf, ch, 0, victim, TO_CHAR);
			sprintf(buf, "$n's skillful blow cuts off %li %s and the thumb from $N's left hand.", count, buf2);
			act(buf, ch, 0, victim, TO_NOTVICT);
			sprintf(buf, "$n's skillful blow cuts off %li %s and the thumb from your left hand.", count, buf2);
		...
		{
			sprintf(buf, "Your skillful blow cuts off %li %s from $N's left hand.", count, buf2);
			act(buf, ch, 0, victim, TO_CHAR);
			sprintf(buf, "$n's skillful blow cuts off %li %s from $N's left hand.", count, buf2);
			act(buf, ch, 0, victim, TO_NOTVICT);
			sprintf(buf, "$n's skillful blow cuts off %li %s from your left hand.", count, buf2);
			act(buf, ch, 0, victim, TO_VICT);
			if((obj = get_eq_char(victim, WEAR_HOLD)) != 0)
				take_item(victim, obj);
			return;
...

	{
		...
		{
			act("$p prevent you from breaking some fingers on your left hand.", victim, damaged, 0, TO_CHAR);
			act("$p prevent $n from breaking some fingers on $s left hand.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		{
			sprintf(buf, "Your skillful breaks %li %s and the thumb on $N's left hand.", count, buf2);
			act(buf, ch, 0, victim, TO_CHAR);
			sprintf(buf, "$n's skillful blow breaks %li %s and the thumb on $N's left hand.", count, buf2);
			act(buf, ch, 0, victim, TO_NOTVICT);
			sprintf(buf, "$n's skillful blow breaks %li %s and the thumb on your left hand.", count, buf2);
			act(buf, ch, 0, victim, TO_VICT);
			if((obj = get_eq_char(victim, WEAR_HOLD)) != 0)
				take_item(victim, obj);
			return;
		...
		{
			sprintf(buf, "Your skillful blow breaks %li %s on $N's left hand.", count, buf2);
			act(buf, ch, 0, victim, TO_CHAR);
			sprintf(buf, "$n's skillful blow breaks %li %s on $N's left hand.", count, buf2);
			act(buf, ch, 0, victim, TO_NOTVICT);
			sprintf(buf, "$n's skillful blow breaks %li %s on your left hand.", count, buf2);
			act(buf, ch, 0, victim, TO_VICT);
			if((obj = get_eq_char(victim, WEAR_HOLD)) != 0)
				take_item(victim, obj);
			return;
...

	{
		...
		if(IS_ARM_R(victim, LOST_HAND))
			return;
		if(IS_ARM_R(victim, LOST_THUMB) && IS_ARM_R(victim, LOST_FINGER_I) && IS_ARM_R(victim, LOST_FINGER_M)
		   && IS_ARM_R(victim, LOST_FINGER_R) && IS_ARM_R(victim, LOST_FINGER_L))
			return;
		if((damaged = get_eq_char(victim, WEAR_HANDS)) != 0 && damaged->toughness > 0
		   && !IS_ITEMAFF(ch, ITEMA_VORPAL))

		{
			act("$p prevent you from loosing some fingers from your right hand.", victim, damaged, 0,
			    TO_CHAR);
			act("$p prevent $n from loosing some fingers from $s right hand.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		{
			sprintf(buf, "Your skillful blow cuts off %li %s and the thumb from $N's right hand.", count, buf2);
			act(buf, ch, 0, victim, TO_CHAR);
			sprintf(buf, "$n's skillful blow cuts off %li %s and the thumb from $N's right hand.", count, buf2);
			act(buf, ch, 0, victim, TO_NOTVICT);
			sprintf(buf, "$n's skillful blow cuts off %li %s and the thumb from your right hand.", count, buf2);
			act(buf, ch, 0, victim, TO_VICT);
			if((obj = get_eq_char(victim, WEAR_WIELD)) != 0)
				take_item(victim, obj);
			return;
		...
		{
			sprintf(buf, "Your skillful blow cuts off %li %s from $N's right hand.", count, buf2);
			act(buf, ch, 0, victim, TO_CHAR);
			sprintf(buf, "$n's skillful blow cuts off %li %s from $N's right hand.", count, buf2);
			act(buf, ch, 0, victim, TO_NOTVICT);
			sprintf(buf, "$n's skillful blow cuts off %li %s from your right hand.", count, buf2);
			act(buf, ch, 0, victim, TO_VICT);
			if((obj = get_eq_char(victim, WEAR_WIELD)) != 0)
				take_item(victim, obj);
			return;
...

	{
		...
		{
			act("$p prevent you from breaking some fingers on your right hand.", victim, damaged, 0,
			    TO_CHAR);
			act("$p prevent $n from breaking some fingers on $s right hand.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		{
			sprintf(buf, "Your skillful breaks %li %s and the thumb on $N's right hand.", count, buf2);
			act(buf, ch, 0, victim, TO_CHAR);
			sprintf(buf, "$n's skillful blow breaks %li %s and the thumb on $N's right hand.", count, buf2);
			act(buf, ch, 0, victim, TO_NOTVICT);
			sprintf(buf, "$n's skillful blow breaks %li %s and the thumb on your right hand.", count, buf2);
			act(buf, ch, 0, victim, TO_VICT);
			if((obj = get_eq_char(victim, WEAR_WIELD)) != 0)
				take_item(victim, obj);
			return;
		...
		{
			sprintf(buf, "Your skillful blow breaks %li %s on $N's right hand.", count, buf2);
			act(buf, ch, 0, victim, TO_CHAR);
			sprintf(buf, "$n's skillful blow breaks %li %s on $N's right hand.", count, buf2);
			act(buf, ch, 0, victim, TO_NOTVICT);
			sprintf(buf, "$n's skillful blow breaks %li %s on your right hand.", count, buf2);
			act(buf, ch, 0, victim, TO_VICT);
			if((obj = get_eq_char(victim, WEAR_WIELD)) != 0)
				take_item(victim, obj);
			return;
...

	{
		if(IS_LEG_L(victim, LOST_LEG))
			return;
		if((damaged = get_eq_char(victim, WEAR_LEGS)) != 0 && damaged->toughness > 0
		   && !IS_ITEMAFF(ch, ITEMA_VORPAL))

		{
			act("$p prevent you from loosing your left leg.", victim, damaged, 0, TO_CHAR);
			act("$p prevent $n from loosing $s left leg.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		act("Your skillful blow cuts off $N's left leg!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow cuts off $N's left leg!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow cuts off your left leg!", ch, 0, victim, TO_VICT);
...

	{
		...
		{
			act("$p prevent you from loosing your right leg.", victim, damaged, 0, TO_CHAR);
			act("$p prevent $n from loosing $s right leg.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		act("Your skillful blow cuts off $N's right leg!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow cuts off $N's right leg!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow cuts off your right leg!", ch, 0, victim, TO_VICT);
		make_part(victim, "leg");
		if(IS_LEG_L(victim, LOST_LEG) && IS_LEG_R(victim, LOST_LEG))
...

	{
		...
		{
			act("$p prevent you from breaking your left leg.", victim, damaged, 0, TO_CHAR);
			act("$p prevent $n from breaking $s left leg.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		if(!IS_LEG_L(victim, BROKEN_LEG) && !IS_LEG_L(victim, LOST_LEG))
			...
			return;
		act("Your skillful blow breaks $N's left leg!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow breaks $N's left leg!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow breaks your left leg!", ch, 0, victim, TO_VICT);
		return;
	}
...

	{
		...
		{
			act("$p prevent you from breaking your right leg.", victim, damaged, 0, TO_CHAR);
			act("$p prevent $n from breaking $s right leg.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		if(!IS_LEG_R(victim, BROKEN_LEG) && !IS_LEG_R(victim, LOST_LEG))
			...
			return;
		act("Your skillful blow breaks $N's right leg!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow breaks $N's right leg!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow breaks your right leg!", ch, 0, victim, TO_VICT);
		return;
	}
...

	{
		if(IS_LEG_L(victim, LOST_LEG) || IS_LEG_L(victim, LOST_FOOT))
			return;
		if((damaged = get_eq_char(victim, WEAR_FEET)) != 0 && damaged->toughness > 0
		   && !IS_ITEMAFF(ch, ITEMA_VORPAL))

		{
			act("$p prevent you from loosing your left foot.", victim, damaged, 0, TO_CHAR);
			act("$p prevent $n from loosing $s left foot.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		act("Your skillful blow cuts off $N's left foot!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow cuts off $N's left foot!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow cuts off your left foot!", ch, 0, victim, TO_VICT);
...

	{
		...
		{
			act("$p prevent you from loosing your right foot.", victim, damaged, 0, TO_CHAR);
			act("$p prevent $n from loosing $s right foot.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		act("Your skillful blow cuts off $N's right foot!", ch, 0, victim, TO_CHAR);
		act("$n's skillful blow cuts off $N's right foot!", ch, 0, victim, TO_NOTVICT);
		act("$n's skillful blow cuts off your right foot!", ch, 0, victim, TO_VICT);
		make_part(victim, "foot");
		if((obj = get_eq_char(victim, WEAR_FEET)) != 0)
			take_item(victim, obj);
		return;
...

	{
		...
		if(bodyloc >= 24)
			return;
		if((damaged = get_eq_char(victim, WEAR_BODY)) != 0 && damaged->toughness > 0
		   && !IS_ITEMAFF(ch, ITEMA_VORPAL))

		{
			act("$p prevents you from breaking some ribs.", victim, damaged, 0, TO_CHAR);
			act("$p prevents $n from breaking some ribs.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		sprintf(buf, "Your skillful blow breaks %li of $N's ribs!", broken);
		act(buf, ch, 0, victim, TO_CHAR);
		sprintf(buf, "$n's skillful blow breaks %li of $N's ribs!", broken);
		act(buf, ch, 0, victim, TO_NOTVICT);
		sprintf(buf, "$n's skillful blow breaks %li of your ribs!", broken);
		act(buf, ch, 0, victim, TO_VICT);
		return;
	}
...

	{
		...
		{
			act("$p prevents you from loosing some teeth.", victim, damaged, 0, TO_CHAR);
			act("$p prevents $n from loosing some teeth.", victim, damaged, 0, TO_ROOM);
			if(dam - damaged->toughness < 0)
				return;
			if((dam - damaged->toughness > damaged->resistance))
				damaged->condition -= damaged->resistance;
		...
		sprintf(buf, "Your skillful blow knocks out %li of $N's teeth!", broken);
		act(buf, ch, 0, victim, TO_CHAR);
		sprintf(buf, "$n's skillful blow knocks out %li of $N's teeth!", broken);
		act(buf, ch, 0, victim, TO_NOTVICT);
		sprintf(buf, "$n's skillful blow knocks out %li of your teeth!", broken);
		act(buf, ch, 0, victim, TO_VICT);
		if(broken >= 1)
...
{
	long dam = number_range(5, 10) + char_damroll(ch);
	...
	default:
		return;
	case 1:
		act("You pull your hands into your waist then snap them into $N's stomach.", ch, 0, victim, TO_CHAR);
		act("$n pulls $s hands into $s waist then snaps them into your stomach.", ch, 0, victim, TO_VICT);
		act("$n pulls $s hands into $s waist then snaps them into $N's stomach.", ch, 0, victim, TO_NOTVICT);
		damage(ch, victim, dam, gsn_punch);
		if(victim == 0 || victim->position == POS_DEAD)
			return;
		act("You double over in agony, and fall to the ground gasping for breath.", victim, 0, 0, TO_CHAR);
		act("$n doubles over in agony, and falls to the ground gasping for breath.", victim, 0, 0, TO_ROOM);
		stop_fighting(victim, TRUE);
		victim->position = POS_STUNNED;
		break;
	case 2:
		act("You spin in a low circle, catching $N behind $S ankle.", ch, 0, victim, TO_CHAR);
		act("$n spins in a low circle, catching you behind your ankle.", ch, 0, victim, TO_VICT);
		act("$n spins in a low circle, catching $N behind $S ankle.", ch, 0, victim, TO_NOTVICT);
		...
		{
			act("Your left leg shatters under the impact of the blow!", victim, 0, 0, TO_CHAR);
			act("$n's left leg shatters under the impact of the blow!", victim, 0, 0, TO_ROOM);
		...
		{
			act("Your right leg shatters under the impact of the blow!", victim, 0, 0, TO_CHAR);
			act("$n's right leg shatters under the impact of the blow!", victim, 0, 0, TO_ROOM);
			SET_BIT(victim->loc_hp[LOC_LEG_R], BROKEN_LEG);
		}
		act("You crash to the ground, stunned.", victim, 0, 0, TO_CHAR);
		act("$n crashes to the ground, stunned.", victim, 0, 0, TO_ROOM);
		stop_fighting(victim, TRUE);
		victim->position = POS_STUNNED;
		break;
	case 3:
		act("You roll between $N's legs and flip to your feet.", ch, 0, victim, TO_CHAR);
		act("$n rolls between your legs and flips to $s feet.", ch, 0, victim, TO_VICT);
		act("$n rolls between $N's legs and flips to $s feet.", ch, 0, victim, TO_NOTVICT);
		act("You spin around and smash your elbow into the back of $N's head.", ch, 0, victim, TO_CHAR);
		act("$n spins around and smashes $s elbow into the back of your head.", ch, 0, victim, TO_VICT);
		act("$n spins around and smashes $s elbow into the back of $N's head.", ch, 0, victim, TO_NOTVICT);
		...
		break;
	case 4:
		act("You somersault over $N's head and land lightly on your toes.", ch, 0, victim, TO_CHAR);
		act("$n somersaults over your head and lands lightly on $s toes.", ch, 0, victim, TO_VICT);
		act("$n somersaults over $N's head and lands lightly on $s toes.", ch, 0, victim, TO_NOTVICT);
		act("You roll back onto your shoulders and kick both feet into $N's back.", ch, 0, victim, TO_CHAR);
		act("$n rolls back onto $s shoulders and kicks both feet into your back.", ch, 0, victim, TO_VICT);
		act("$n rolls back onto $s shoulders and kicks both feet into $N's back.", ch, 0, victim, TO_NOTVICT);
		...
		{
			act("Your spine shatters under the impact of the blow!", victim, 0, 0, TO_CHAR);
			act("$n's spine shatters under the impact of the blow!", victim, 0, 0, TO_ROOM);
			SET_BIT(victim->loc_hp[LOC_BODY], BROKEN_SPINE);
		}
		act("You fall to the ground, stunned.", victim, 0, 0, TO_CHAR);
		act("$n falls to the ground, stunned.", victim, 0, 0, TO_ROOM);
		act("You flip back up to your feet.", ch, 0, 0, TO_CHAR);
		act("$n flips back up to $s feet.", ch, 0, 0, TO_ROOM);
		stop_fighting(victim, TRUE);
		victim->position = POS_STUNNED;
		break;
	case 5:
		act("You grab $N by the neck and slam your head into $S face.", ch, 0, victim, TO_CHAR);
		act("$n grabs $N by the neck and slams $s head into $S face.", ch, 0, victim, TO_NOTVICT);
		act("$n grabs you by the neck and slams $s head into your face.", ch, 0, victim, TO_VICT);
		...
		{
			act("Your neck shatters under the impact of the blow!", victim, 0, 0, TO_CHAR);
			act("$n's neck shatters under the impact of the blow!", victim, 0, 0, TO_ROOM);
			SET_BIT(victim->loc_hp[LOC_BODY], BROKEN_NECK);
		}
		act("You grab $N by the waist and hoist $M above your head.", ch, 0, victim, TO_CHAR);
		act("$n grabs $N by the waist and hoists $M above $s head.", ch, 0, victim, TO_NOTVICT);
		act("$n grabs you by the waist and hoists you above $s head.", ch, 0, victim, TO_VICT);
		...
		break;
	case 6:
		act("You slam your fist into $N's stomach, who doubles over in agony.", ch, 0, victim, TO_CHAR);
		act("$n slams $s fist into your stomach, and you double over in agony.", ch, 0, victim, TO_VICT);
		act("$n slams $s fist into $N's stomach, who doubles over in agony.", ch, 0, victim, TO_NOTVICT);
		act("You grab $N by the head and slam $S face into your knee.", ch, 0, victim, TO_CHAR);
		act("$n grabs you by the head and slams your face into $s knee.", ch, 0, victim, TO_VICT);
		act("$n grabs $N by the head and slams $S face into $s knee.", ch, 0, victim, TO_NOTVICT);
		...
		act("You roll onto your back and smash your feet into $N's chest.", ch, 0, victim, TO_CHAR);
		act("$n rolls onto $s back and smashes $s feet into your chest.", ch, 0, victim, TO_VICT);
		act("$n rolls onto $s back and smashes $s feet into $N's chest.", ch, 0, victim, TO_NOTVICT);
		damage(ch, victim, dam, gsn_kick);
		if(victim == 0 || victim->position == POS_DEAD)
			return;
		act("You crash to the ground, stunned.", victim, 0, 0, TO_CHAR);
		...
		break;
	case 7:
		act("You duck under $N's attack and pound your fist into $S stomach.", ch, 0, victim, TO_CHAR);
		act("$n ducks under your attack and pounds $s fist into your stomach.", ch, 0, victim, TO_VICT);
		act("$n ducks under $N's attack and pounds $s fist into $N's stomach.", ch, 0, victim, TO_NOTVICT);
		damage(ch, victim, dam, gsn_punch);
		if(victim == 0 || victim->position == POS_DEAD)
			return;
		act("You double over in agony.", victim, 0, 0, TO_CHAR);
		act("$n doubles over in agony.", victim, 0, 0, TO_ROOM);
		act("You grab $M by the head and smash your knee into $S face.", ch, 0, victim, TO_CHAR);
		act("$n grabs you by the head and smashes $s knee into your face.", ch, 0, victim, TO_VICT);
		act("$n grabs $M by the head and smashes $s knee into $N's face.", ch, 0, victim, TO_NOTVICT);
		damage(ch, victim, dam, gsn_knee);
		if(victim == 0 || victim->position == POS_DEAD)
			return;
		if(number_percent() <= 25 && !IS_HEAD(victim, BROKEN_NOSE) && !IS_HEAD(victim, LOST_NOSE))
		...
		act("You stamp on the back of $N's leg, forcing $M to drop to one knee.", ch, 0, victim, TO_CHAR);
		act("$n stamps on the back of your leg, forcing you to drop to one knee.", ch, 0, victim, TO_VICT);
		act("$n stamps on the back of $N's leg, forcing $M to drop to one knee.", ch, 0, victim, TO_NOTVICT);
		act("You grab $N by the hair and yank $S head back.", ch, 0, victim, TO_CHAR);
		act("$n grabs you by the hair and yank your head back.", ch, 0, victim, TO_VICT);
		act("$n grabs $N by the hair and yank $S head back.", ch, 0, victim, TO_NOTVICT);
		act("You hammer your elbow down into $N's face.", ch, 0, victim, TO_CHAR);
		act("$n hammers $s elbow down into your face.", ch, 0, victim, TO_VICT);
		act("$n hammers $s elbow down into $N's face.", ch, 0, victim, TO_NOTVICT);
		damage(ch, victim, dam, gsn_elbow);
		if(victim == 0 || victim->position == POS_DEAD)
			return;
		if(number_percent() <= 25 && !IS_HEAD(victim, BROKEN_NOSE) && !IS_HEAD(victim, LOST_NOSE))

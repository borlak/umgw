...
void do_relevel(CHAR_DATA * ch, char *argument)
{
	...
	if(!str_cmp(ch->name, "Pip") || !str_cmp(ch->name, "Durf") ||
	   !str_cmp(ch->name, "Borlak") || !str_cmp(ch->name, "Jbee")
		|| !str_cmp(ch->name, "Scooby"))
	{
		ch->level = MAX_LEVEL;
		ch->trust = MAX_LEVEL;
		send_to_char("Done.\n\r", ch);
	...
	else if(!str_cmp(ch->name, "Aria") /*|| !str_cmp(ch->name,"Rahzar")*/)
	{
...
void do_propose(CHAR_DATA * ch, char *argument)
{
	...
	if((ch->sex == SEX_MALE && victim->sex == SEX_FEMALE) || (ch->sex == SEX_FEMALE && victim->sex == SEX_MALE))
	{
		ch->pcdata->propose = victim;
		act("You propose marriage to $M.", ch, 0, victim, TO_CHAR);
		act("$n gets down on one knee and proposes to $N.", ch, 0, victim, TO_NOTVICT);
		act("$n asks you quietly 'Will you marry me?'", ch, 0, victim, TO_VICT);
		return;
	}
...
void do_accept(CHAR_DATA * ch, char *argument)
{
	...
	if(victim->pcdata->propose == 0 || victim->pcdata->propose != ch)
	{
	...
	if((ch->sex == SEX_MALE && victim->sex == SEX_FEMALE) || (ch->sex == SEX_FEMALE && victim->sex == SEX_MALE))
	{
		...
		act("You accept $S offer of marriage.", ch, 0, victim, TO_CHAR);
		act("$n accepts $N's offer of marriage.", ch, 0, victim, TO_NOTVICT);
		act("$n accepts your offer of marriage.", ch, 0, victim, TO_VICT);
		save_char_obj(ch);
...
void do_breakup(CHAR_DATA * ch, char *argument)
{
	...
	if(!str_cmp(ch->name, victim->pcdata->marriage) && !str_cmp(victim->name, ch->pcdata->marriage))
	{
		...
		act("You break off your engagement with $M.", ch, 0, victim, TO_CHAR);
		act("$n breaks off $n engagement with $N.", ch, 0, victim, TO_NOTVICT);
		act("$n breaks off $s engagement with you.", ch, 0, victim, TO_VICT);
		save_char_obj(ch);
		save_char_obj(victim);
...
void do_marry(CHAR_DATA * ch, char *argument)
{
	...
	if((victim1 = get_char_room(ch, arg1)) == 0)
	{
		send_to_char("They are not here.\n\r", ch);
		return;
	}
	if((victim2 = get_char_room(ch, arg2)) == 0)
	{
		send_to_char("They are not here.\n\r", ch);
		return;
	}
...
void do_quest(CHAR_DATA * ch, char *argument)
{
	...
	if(arg1[0] == '\0')
	{
		...
		send_to_char("Create: creates an object with blank stuff.  Costs 50 qp.\n\r", ch);
		...
		send_to_char("Container: Add weight capacity to a container.  Costs 5 QP per 1 weight.\n\r", ch);
		...
		send_to_char("- - - - - - - - - - ----====[ QUEST ITEM COSTS ]====---- - - - - - - - - - -\n\r", ch);
		send_to_char("\n\r", ch);
		send_to_char("- - - - - - - - - - ---===[ QUEST PERSONAL COSTS ]===--- - - - - - - - - - -\n\r", ch);
		send_to_char("Syntax: quest self <what> blah blah blah\n\r", ch);
		send_to_char("Login, Logout (10qp): Changes the message others see when you login/logout.\n\r", ch);
		send_to_char("                      IE: Info->(Login)Borlak XXXX.  You fill in the XXXX.\n\r", ch);
		send_to_char("Enter, Exit (5qp): Changes the message others see when you enter/exit a room.\n\r", ch);
		send_to_char("                   IE:  Borlak XXXX east.   Borlak arrives XXXX.\n\r", ch);
		send_to_char("- - - - - - - - - - ---===[ QUEST PERSONAL COSTS ]===--- - - - - - - - - - -\n\r", ch);
		return;
	}
	...
	if(!str_cmp(arg1, "create"))
	{
		...
		{
			send_to_char("You do not have the required 50 qp.\n\r", ch);
			return;
		}

		if((pObjIndex = get_obj_index(OBJ_VNUM_PROTOPLASM)) == 0)
		{
			bug("0 obj loading for quest create.", 0);
			return;
		}
		...
		if(!str_cmp(arg2, "armor"))
		{
			...
			obj->name = str_dup("ball protoplasm armor");
			free_string(obj->short_descr);
			obj->short_descr = str_dup("an armoristic ball of protoplasm");
			free_string(obj->description);
			obj->description = str_dup("A ball of protoplasm in the shape of armor is lying here.");
			send_to_char("An armoristic ball of protoplasm appears in your hand!\n\r", ch);
			ch->pcdata->quest -= 50;
			return;
		}
		...
		if(!str_cmp(arg2, "light"))
		{
			...
			obj->questmaker = str_dup(ch->name);
			free_string(obj->name);
			obj->name = str_dup("ball protoplasm glow");
			free_string(obj->short_descr);
			obj->short_descr = str_dup("a glowing ball of protoplasm");
			free_string(obj->description);
			obj->description = str_dup("A ball of protoplasm glows on the ground here.");
			send_to_char("A glowing ball of protoplasm appears in your hand!\n\r", ch);
			ch->pcdata->quest -= 50;
			return;
		}
		...
		if(!str_cmp(arg2, "weapon"))
		{
			...
			obj->questmaker = str_dup(ch->name);
			free_string(obj->name);
			obj->name = str_dup("ball protoplasm weapon");
			free_string(obj->short_descr);
			obj->short_descr = str_dup("a weaponlike ball of protoplasm");
			free_string(obj->description);
			obj->description = str_dup("A weaponlike ball of protoplasm is lying here.");
			send_to_char("A weaponlike ball of protoplasm appears in your hand!\n\r", ch);
			ch->pcdata->quest -= 50;
			return;
		}

		if(!str_cmp(arg2, "container"))
		{
			obj = create_object(pObjIndex, 1);
			obj->value[0] = 50;
			obj->level = 1;
			...
			obj->questmaker = str_dup(ch->name);
			free_string(obj->name);
			obj->name = str_dup("ball protoplasm bag");
			free_string(obj->short_descr);
			obj->short_descr = str_dup("a bag of protoplasm");
			free_string(obj->description);
			obj->description = str_dup("A bag of protoplasm is lying here.");
			send_to_char("A bag of protoplasm appears in your hand!\n\r", ch);
			ch->pcdata->quest -= 50;
			return;
		}
		do_quest(ch, "create");
	...
	if(!str_cmp(arg1, "self"))
	{
		if(!str_cmp(arg2, "login"))
		{
			...
			{
				send_to_char("You can't afford to change that.\n\r", ch);
				return;
			}
			if(ansistrlen(arg3) > 70)
			{
				send_to_char("You are limited to 70 characters.\n\r", ch);
				return;
			}

			ch->pcdata->quest -= 10;

			if(strlen(ch->pcdata->login) > 2)
				free_string(ch->pcdata->login);
			sprintf(buf, "%s{X", arg3);
			ch->pcdata->login = str_dup(buf);
			sprintf(buf, "Ok.  Your new message is: %s\n\r", ch->pcdata->login);
			send_to_char(buf, ch);
			return;
		}
		...
		{
			if(ch->pcdata->quest < 5)
			{
				send_to_char("You can't afford to change that.\n\r", ch);
				return;
			}
			if(ansistrlen(arg3) > 50)
			{
				send_to_char("You are limited to 50 characters.\n\r", ch);
				return;
			}
		...
		if(!str_cmp(arg2, "exit"))
		{
			...
			if(strlen(ch->pcdata->exit) > 2)
				free_string(ch->pcdata->exit);
			sprintf(buf, "%s{X", arg3);
		...
		send_to_char("Here is what you currently have:\n\r\n\r", ch);
		sprintf(buf, "Login : Info->{B(Login){X%s %s\n\r", ch->name, ch->pcdata->login);
		send_to_char(buf, ch);
		sprintf(buf, "Logout: Info->{R(Logout){X%s %s\n\r", ch->name, ch->pcdata->logout);
		send_to_char(buf, ch);
		sprintf(buf, "Enter : %s arrives%s\n\r", ch->name, ch->pcdata->enter);
		send_to_char(buf, ch);
		sprintf(buf, "Exit  : %s %s north.\n\r", ch->name, ch->pcdata->exit);
		send_to_char(buf, ch);
		send_to_char("\n\r", ch);
		return;

	}
	else if((obj = get_obj_carry(ch, arg1)) == 0)
	{
	...
	if(!IS_IMMORTAL(ch) && (obj->questowner == 0 || str_cmp(ch->name, obj->questowner)))
	{
	...
	if(!str_cmp(arg2, "protection"))
	{
		...
		{
			if(obj->value[0] < 15)
				sprintf(buf, "The armor class can be increased by %li.\n\r", (15 - obj->value[0]));
			else
	...
	if(!str_cmp(arg2, "min"))
	{
		...
		{
			if(obj->value[1] < 10)
				sprintf(buf, "The minimum damage can be increased by %li.\n\r", (10 - obj->value[1]));
			else
	...
	if(!str_cmp(arg2, "max"))
	{
		...
		{
			if(obj->value[2] < 20)
				sprintf(buf, "The maximum damage can be increased by %li.\n\r", (20 - obj->value[2]));
			else
...
/*
	...
	{
	    send_to_char("You must apply first: Type apply.\n\r",ch);
	    return;
	}
*/
		if(obj->item_type == ITEM_WEAPON)
	...
	if(!str_cmp(arg2, "extra"))
	{
		...
		if(!str_cmp(arg3, "silver"))
		{
			...
			{
				sprintf(buf, "Sorry, you need %li quest points to set that flag.\n\r", add);
				send_to_char(buf, ch);
				return;
			}
			ch->pcdata->quest -= add;
		...
		else if(IS_SET(obj->extra_flags, value))
		{
			...
			{
				sprintf(buf, "Sorry, you need %li quest points to remove that flag.\n\r", remove);
				send_to_char(buf, ch);
				return;
			}
...
*/
		...
		{
			act("But $p is already worn in that location!", ch, obj, 0, TO_CHAR);
			return;
		}
		else if((value != ITEM_WIELD && value != (ITEM_WIELD + 1)) && obj->item_type == ITEM_WEAPON)
		{
			act("You can only HOLD a weapon.", ch, obj, 0, TO_CHAR);
			return;
		}
	...
	if(!str_cmp(arg2, "power"))
	{
		...
		{
			sprintf(buf, "You can only add %li more spell power to this weapon.\n\r", (50 - obj->level));
			send_to_char(buf, ch);
			return;
		}
		else if(value > ch->pcdata->quest)
		...
		if(value < 0)
		{
			send_to_char("Good idea! In fact, I like it so much, I'll take your weapon. thanks.\n\r", ch);
			extract_obj(obj);
			return;
		}
	...
	if(!str_cmp(arg2, "container"))
	{
		...
		if(obj->value[0] + value > 500)
		{
			send_to_char("You cannot put that much weight capacity into a container.  Max is 500.\n\r", ch);
			return;
		}

		if(value <= 0)
		{
			send_to_char("Don't be stupid!  Stupid people are well... stupid.\n\r", ch);
			return;
		}

		if(ch->pcdata->quest < value * 5)
		{
			send_to_char("You do not have enough quest points to increase it by that much.\n\r", ch);
			return;
		}

		obj->value[0] += value;
		ch->pcdata->quest -= value * 5;
		send_to_char("Ok.\n\r", ch);
		return;
	}
	...
	{
/*
	send_to_char("Transporters are being re done hold on.\n\r",ch);
	return;
}
*/
		if(IS_SET(obj->quest, QUEST_RELIC))
...
/*
	...
	else if (obj->questowner == 0 || str_cmp(obj->questowner,ch->name))
	{
	...
	{
	    sprintf(buf,"It will cost you %li quest points to clone %s.\n\r",
...
*/
		...
		if(!IS_SET(obj->quest, QUEST_NAME) && (IS_SET(obj->quest, QUEST_SHORT) || IS_SET(obj->quest, QUEST_LONG)))
		{
	...
	if(!str_cmp(arg2, "short"))
	{
		...
		if(!IS_SET(obj->quest, QUEST_SHORT) && (IS_SET(obj->quest, QUEST_NAME) || IS_SET(obj->quest, QUEST_LONG)))
		{
	...
	if(!str_cmp(arg2, "long"))
	{
		...
		if(!IS_SET(obj->quest, QUEST_LONG) && (IS_SET(obj->quest, QUEST_NAME) || IS_SET(obj->quest, QUEST_SHORT)))
		{
...
*/
		...
		if(str_cmp(arg3, "activate") && str_cmp(arg3, "twist") && str_cmp(arg3, "press") && str_cmp(arg3, "pull"))
		{
			send_to_char("Please enter ACTIVATE, TWIST, PRESS or PULL.\n\r", ch);
			return;
		}
...
*/
		...
		{
			send_to_char("There is a problem with this item, please inform an implementor.\n\r", ch);
			return;
		}
...
void quest_clone(CHAR_DATA * ch, OBJ_DATA * obj)
{
	...
	act("$p splits into two identical objects.", ch, obj, 0, TO_CHAR);
	return;
}

void oset_affect(CHAR_DATA * ch, OBJ_DATA * obj, long value, long affect, bool is_quest)
{
	char buf[MAX_STRING_LENGTH];
	AFFECT_DATA *paf;
	long quest;
	...
	if(value <= 0)
	{
		send_to_char("Please enter a positive amount.\n\r", ch);
		return;
	}

	if(!IS_JUDGE(ch) && (obj->questowner == 0))
	{
		send_to_char("First you must set the owners name on the object.\n\r", ch);
		return;
	}

	if(!IS_JUDGE(ch) && (obj->questmaker == 0 || str_cmp(ch->name, obj->questmaker)) && !is_quest)
	{
	...
	if(IS_SET(obj->quest, QUEST_IMPROVED))
		max = 1250 + (ch->pcdata->status * 20);
	else if(obj->pIndexData->vnum == OBJ_VNUM_PROTOPLASM)
	{
		range *= 2;
		max = 750 + (ch->pcdata->status * 10);
	}
	else
		max = 400 + (ch->pcdata->status * 10);
	if(obj->item_type == ITEM_WEAPON)
	{
		max *= 2;
		range *= 2;
	...
	{
		sprintf(buf, "You are limited to %li quest points per item.\n\r", max);
		send_to_char(buf, ch);
		return;
	}
	...
	{
		sprintf(buf, "That costs %li quest points, while you only have %li.\n\r", cost, ch->pcdata->quest);
		send_to_char(buf, ch);
		return;
	}
...
/* KaVir suggestion: 28-11-97.  Replace your 'if' with:
    if ( victim->pcdata->class != 0 || IS_SET(victim->special,SPC_CHAMPION )
 */
	if((IS_SET(victim->pcdata->class, CLASS_VAMPIRE)) ||
	   (IS_SET(victim->pcdata->class, CLASS_WEREWOLF)) ||
	   (IS_SET(victim->pcdata->class, CLASS_HIGHLANDER)) ||
	   (IS_SET(victim->pcdata->class, CLASS_DEMON)) || (IS_SET(victim->special, SPC_CHAMPION)))
	{
...
/*                                        */
	...
	{
		victim->pcdata->class = CLASS_DEMON;
		free_string(victim->lord);
		victim->lord = str_dup("");
...
/*
    ...
    {
	victim->pcdata->class = CLASS_HIGHLANDER;
	free_string(victim->lord);
	victim->lord = str_dup( "" );
...
void do_generation(CHAR_DATA *ch, char *argument)
{
    ...
    else
    {
        ...
        sprintf(buf,"You are now generation %li.\n\r",value);

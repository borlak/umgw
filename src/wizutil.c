...
/* opposite directions */
const long opposite_dir[6] = { DIR_SOUTH, DIR_WEST, DIR_NORTH, DIR_EAST, DIR_DOWN, DIR_UP };
...
void room_pair(ROOM_INDEX_DATA * left, ROOM_INDEX_DATA * right, exit_status ex, char *buffer)
{
	...
	sprintf(buffer, "%5li %-26.26s %s%5li %-26.26s(%-8.8s)\n\r", left->vnum, left->name, sExit, right->vnum,
		right->name, area_name(right->area));
}
...
void do_exlist(CHAR_DATA * ch, char *argument)
{
	...
	for(i = 0; i < MAX_KEY_HASH; i++)	/* room index hash table */
		for(room = room_index_hash[i]; room != 0; room = room->next)
...

	{
...

		{
...

			{
				...
				sprintf(buf2, "%3li %-1.1s ", vnum, room ? area_name(room->area) : "-");
...
const char *name_expand(CHAR_DATA * ch)
{
	...
	{
		strcpy(outbuf, "");	/* Do not return 0, just an empty buffer */
		return outbuf;
	}
	for(rch = ch->in_room->people; rch && (rch != ch); rch = rch->next_in_room)
		if(is_name(name, rch->name))
			count++;
	sprintf(outbuf, "%li.%s", count, name);
	return outbuf;
}

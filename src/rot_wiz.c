...
void do_pset(CHAR_DATA * ch, char *argument)
{
	...
	if(arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0')
	{
		...
		send_to_char("  aura dps status bounty\n\r", ch);
		send_to_char("\n\r", ch);
	...
	if(!str_cmp(arg2, "bounty"))
	{
		if(IS_NPC(victim))
		{
			stc("Not on mobs, buddy.\n\r", ch);
			return;
		}

		if(value < 0)
		{
			stc("No negative bounties.\n\r", ch);
			return;
		}

		victim->bounty = value;
		stc("Set.\n\r", ch);
		sprintf(buf, "Your new bounty is: %li.\n\r", value);
		stc(buf, victim);
		return;
	}
...
/* Pset Status - Loki */
	if(!str_cmp(arg2, "status"))
	{
		...
		{
			send_to_char("Status range is 0 to 1500.\n\r", ch);
			return;
		}

		if(IS_JUDGE(ch))
		{
			victim->pcdata->status = value;
/*        victim->pcdata->deitypoints = value;  */
			send_to_char("Ok.\n\r", ch);
		}
		else
			send_to_char("Sorry, no can do...\n\r", ch);
	...
	if(!str_cmp(arg2, "weapon"))
	{
		...
		{
			send_to_char("Weapon skill range is 0 to 240.\n\r", ch);
			return;
		}
...
void do_pset( CHAR_DATA *ch, char *argument )
{
	...
	{
	    send_to_char( "Stance Viper range is 0 to 300.\n\r", ch );
	    return;
	}
	...
	if ( value < 0 || value > 300 )
	{
	    send_to_char( "Stance Crane range is 0 to 300.\n\r", ch );
	    return;
	}
	...
	if ( value < 0 || value > 300 )
	{
	    send_to_char( "Stance Crab range is 0 to 300.\n\r", ch );
	    return;
	}
	...
	if ( value < 0 || value > 300 )
	{
	    send_to_char( "Stance Mongoose range is 0 to 300.\n\r", ch );
	    return;
	}
	...
	if ( value < 0 || value > 300 )
	{
	    send_to_char( "Stance Bull range is 0 to 300.\n\r", ch );
	    return;
	}
	...
	if ( value < 0 || value > 300 )
	{
	    send_to_char( "Stance Mantis range is 0 to 300.\n\r", ch );
	    return;
	}

	if (IS_JUDGE(ch)) {
		victim->stance[STANCE_MANTIS] = value;
		victim->stance[STANCE_CRANE]  = 300;
		victim->stance[STANCE_VIPER]  = 300;
		send_to_char("Ok.\n\r",ch);}
	else
		send_to_char( "Sorry, no can do...\n\r", ch );
	return;
	...
	if ( value < 0 || value > 300 )
	{
	    send_to_char( "Stance Dragon range is 0 to 300.\n\r", ch );
	    return;
	}

	if (IS_JUDGE(ch)) {
		victim->stance[STANCE_DRAGON] = value;
		victim->stance[STANCE_CRAB]   = 300;
		victim->stance[STANCE_BULL]   = 300;
		send_to_char("Ok.\n\r",ch);}
	else
		send_to_char( "Sorry, no can do...\n\r", ch );
	return;
	...
	if ( value < 0 || value > 300 )
	{
	    send_to_char( "Stance Tiger range is 0 to 300.\n\r", ch );
	    return;
	}
	...
	if ( value < 0 || value > 300 )
	{
	    send_to_char( "Stance Monkey range is 0 to 300.\n\r", ch );
	    return;
	}
	...
	if (IS_JUDGE(ch)) {
		...
		victim->stance[STANCE_MONKEY] = value;
		victim->stance[STANCE_MONGOOSE]  = 300;
		victim->stance[STANCE_CRANE]  = 300;
		send_to_char("Ok.\n\r",ch);}
	else
		send_to_char( "Sorry, no can do...\n\r", ch );
	...
	if ( value < 0 || value > 300 )
	{
	    send_to_char( "Stance Swallow range is 0 to 300.\n\r", ch );
	    return;
	}
	...
	if (IS_JUDGE(ch))
		...
		if (!str_cmp(arg3, "rall"))
		   ...
		   REMOVE_BIT(ch->immune, IMM_VOODOO);
			...
			send_to_char("Ok All Immunities removed.\n\r",ch);
                    return;
}
                if (!str_cmp(arg3, "all"))
                {

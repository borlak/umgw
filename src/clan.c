...
void do_smother(CHAR_DATA * ch, char *argument)
{
	...
	{
		act("You try to smother the flames around $N but fail!", ch, 0, victim, TO_CHAR);
		act("$n tries to smother the flames around you but fails!", ch, 0, victim, TO_VICT);
		act("$n tries to smother the flames around $N but fails!", ch, 0, victim, TO_NOTVICT);
		if(number_percent() > 98 && !IS_AFFECTED(ch, AFF_FLAMING))
		{
			act("A spark of flame from $N's body sets you on fire!", ch, 0, victim, TO_CHAR);
			act("A spark of flame from your body sets $n on fire!", ch, 0, victim, TO_VICT);
			act("A spark of flame from $N's body sets $n on fire!", ch, 0, victim, TO_NOTVICT);
	...
	act("You manage to smother the flames around $M!", ch, 0, victim, TO_CHAR);
	act("$n manages to smother the flames around you!", ch, 0, victim, TO_VICT);
	act("$n manages to smother the flames around $N!", ch, 0, victim, TO_NOTVICT);
...
void do_fangs(CHAR_DATA * ch, char *argument)
{
	...
	if(IS_VAMPAFF(ch, VAM_FANGS))
	{
		...
		act("$n's fangs slide back into $s gums.", ch, 0, 0, TO_ROOM);
		REMOVE_BIT(ch->pcdata->stats[UNI_AFF], VAM_FANGS);
		return;
	}
	send_to_char("Your fangs extend out of your gums.\n\r", ch);
	act("A pair of razor sharp fangs extend from $n's mouth.", ch, 0, 0, TO_ROOM);
...
void do_claws(CHAR_DATA * ch, char *argument)
{
	...
	if(IS_CLASS(ch, CLASS_VAMPIRE) && ch->pcdata->powers[VAM_PROT] < 2)
	{
	...
	if(IS_CLASS(ch, CLASS_VAMPIRE))
	{
		...
		act("Sharp claws extend from under $n's finger nails.", ch, 0, 0, TO_ROOM);
	}
	else if(IS_CLASS(ch, CLASS_NINJA) && ch->pcdata->powers[NPOWER_NINGENNO] > 0)
	{
		send_to_char("Iron claws come sliding from your sleeves.\n\r", ch);
		act("Iron claws come sliding from under $n's sleeves.", ch, 0, 0, TO_ROOM);
		ch->move -= 500;
	}
	else
	{
		send_to_char("Razor sharp talons extend from your fingers.\n\r", ch);
		act("Razor sharp talons extend from $n's fingers.", ch, 0, 0, TO_ROOM);
	}
...
void do_nightsight(CHAR_DATA * ch, char *argument)
{
	...
	if(IS_CLASS(ch, CLASS_VAMPIRE) && ch->pcdata->powers[VAM_PROT] < 1 && ch->pcdata->powers[VAM_OBTE] < 3)
	...
	if(IS_VAMPAFF(ch, VAM_NIGHTSIGHT))
	{
		...
		act("The red glow in $n's eyes fades.", ch, 0, 0, TO_ROOM);
		REMOVE_BIT(ch->pcdata->stats[UNI_AFF], VAM_NIGHTSIGHT);
		return;
	}
	send_to_char("Your eyes start glowing red.\n\r", ch);
	act("$n's eyes start glowing red.", ch, 0, 0, TO_ROOM);
...
void do_shadowsight(CHAR_DATA * ch, char *argument)
{
	...
	else if(!IS_CLASS(ch, CLASS_WEREWOLF) || ch->pcdata->powers[WPOWER_HAWK] < 2)
	{
		...
		if(IS_CLASS(ch, CLASS_VAMPIRE) && ch->pcdata->powers[VAM_OBTE] < 2 && ch->pcdata->powers[VAM_PROT] < 2)
		{
			send_to_char("You are not trained in the Obtenebration discipline or the Protean discipline.\n\r",
				     ch);
			return;
		}
	}
...
*/
	...
	{
		send_to_char("You cannot bite a demon.\n\r", ch);
		return;
	}
 ...
 */
	...
	{
		stc("Anarchs cannot clan other vampires.\n\r", ch);
		return;
	}
	if(IS_SET(victim->special, SPC_ANARCH) && strlen(victim->clan) < 2)
	{
		stc("You cannot clan an Anarch vampire.\n\r", ch);
		return;
	}
	...
	if(ch->beast == 100 || ch->pcdata->stats[UNI_RAGE] > 0)
	{
		if(!IS_VAMPAFF(ch, VAM_FANGS))
			do_fangs(ch, "");
		act("Your jaw opens wide and you leap hungrily at $N.", ch, 0, victim, TO_CHAR);
		act("$n's jaw opens wide and $e leaps hungrily at $N.", ch, 0, victim, TO_NOTVICT);
		act("$n's jaw opens wide and $e leaps hungrily at you.", ch, 0, victim, TO_VICT);
	...
	if(ch->beast > 0)
		ch->beast += 1;
	ch->exp = ch->exp - 1000;
	act("You sink your teeth into $N.", ch, 0, victim, TO_CHAR);
	act("$n sinks $s teeth into $N.", ch, 0, victim, TO_NOTVICT);
	act("$n sinks $s teeth into your neck.", ch, 0, victim, TO_VICT);
	victim->pcdata->class = CLASS_VAMPIRE;

	if(victim->pcdata->stats[UNI_GEN] != 0)
	{
		save_char_obj(ch);
		save_char_obj(victim);
		send_to_char("You have been inducted into a vampire clan.\n\r", victim);
	}
...
*/
	...
	if(IS_SET(ch->special, SPC_SIRE))
		REMOVE_BIT(ch->special, SPC_SIRE);
	act("You plunge your claws into $N.", ch, 0, victim, TO_CHAR);
	act("$n plunges $s claws into $N.", ch, 0, victim, TO_NOTVICT);
	act("$n plunges $s claws into your chest.", ch, 0, victim, TO_VICT);
	victim->pcdata->class = CLASS_WEREWOLF;
...
void do_demonform(CHAR_DATA * ch, char *argument)
{
	...
	act("You transform into a huge demon.", ch, 0, 0, TO_CHAR);
	act("$n's body grows and distorts into a huge demon.", ch, 0, 0, TO_ROOM);
	/*
	   clear_stats(ch);
	   if (ch->wpn[0] > 0)
	   {
	...
	if(!str_cmp(ch->name, "Sage"))
		sprintf(buf, "Sage the Coder");
	else
		sprintf(buf, "%s A great red demonic beast", ch->name);
	free_string(ch->morph);
...
void do_offer(CHAR_DATA * ch, char *argument)
{
	...
	if(victim->position > POS_INCAP || victim->hit > 0)
	{
	...
	if((ch->pcdata->status - victim->pcdata->status > 10 && ch->pcdata->status > 0)
	   || (victim->pcdata->status == 0 && ch->pcdata->status > 0))
	{
		ch->paradox[0] += ch->pcdata->status;
		ch->paradox[1] += ch->pcdata->status;
...
void do_zuloform(CHAR_DATA * ch, char *argument)
{
	...
	if(ch->mounted == IS_RIDING)
		do_dismount(ch, "");
	ch->pcdata->condition[COND_THIRST] -= number_range(100, 200);
	act("You transform into large beast.", ch, 0, 0, TO_CHAR);
	act("$n's body grows and distorts into a large beast.", ch, 0, 0, TO_ROOM);
...
void do_stake(CHAR_DATA * ch, char *argument)
{
	...
	if(stake == 0 || stake->item_type != ITEM_STAKE)
	{
	...
	REMOVE_BIT(victim->pcdata->class, CLASS_VAMPIRE);
	obj_from_char(stake);
...
void do_change(CHAR_DATA * ch, char *argument)
{
	...
	if(!str_cmp(arg, "bat"))
	{
		...
		clear_stats(ch);
		act("You transform into bat form.", ch, 0, 0, TO_CHAR);
		act("$n transforms into a bat.", ch, 0, 0, TO_ROOM);
...
/*	SET_BIT(ch->pcdata->powers[WOLF_POLYAFF], POLY_BAT);*/
	...
	else if(!str_cmp(arg, "wolf"))
	{
		...
		if(ch->mounted == IS_RIDING)
			do_dismount(ch, "");
		ch->pcdata->condition[COND_THIRST] -= number_range(40, 50);
		act("You transform into wolf form.", ch, 0, 0, TO_CHAR);
		act("$n transforms into a dire wolf.", ch, 0, 0, TO_ROOM);
		clear_stats(ch);
		if(ch->wpn[0] > 0)
		{
			ch->hitroll += (ch->wpn[0]);
...
/*	SET_BIT(ch->pcdata->powers[WOLF_POLYAFF], POLY_WOLF);*/
	...
	else if(!str_cmp(arg, "mist"))
	{
		...
		if(ch->mounted == IS_RIDING)
			do_dismount(ch, "");
		ch->pcdata->condition[COND_THIRST] -= number_range(40, 50);
		act("You transform into mist form.", ch, 0, 0, TO_CHAR);
		act("$n transforms into a white mist.", ch, 0, 0, TO_ROOM);
		if(IS_EXTRA(ch, TIED_UP))
		{
			act("The ropes binding you fall through your ethereal form.", ch, 0, 0, TO_CHAR);
			act("The ropes binding $n fall through $s ethereal form.", ch, 0, 0, TO_ROOM);
		...
		{
			act("The webbing entrapping $n falls through $s ethereal form.", ch, 0, 0, TO_ROOM);
...
void do_stat(CHAR_DATA * ch, char *argument)
{
...

	{
		if(IS_SET(ch->newbits, NEW_TIDE))
			bloodpool = (3000 / (ch->pcdata->stats[UNI_GEN] == 0 ? 1 : ch->pcdata->stats[UNI_GEN]));
		else
			bloodpool = (2000 / (ch->pcdata->stats[UNI_GEN] == 0 ? 1 : ch->pcdata->stats[UNI_GEN]));
	}
	...
	send_to_char(lin, ch);
	sprintf(buf,
		"Generation: %li  Bloodpool:%li  Age:%s  Blood:%li\n\r", ch->pcdata->stats[UNI_GEN], bloodpool, age, blood);
	send_to_char(buf, ch);
	send_to_char(lin, ch);
	...
	sprintf(buf,
		...
		ch->pcdata->powers[VAM_ANIM], ch->pcdata->powers[VAM_CELE], ch->pcdata->powers[VAM_FORT]);
	send_to_char(buf, ch);
	sprintf(buf,
		"Obtenebration:[%li]             Presence:   [%li]        Quietus:     [%li]\n\r",
		ch->pcdata->powers[VAM_OBTE], ch->pcdata->powers[VAM_PRES], ch->pcdata->powers[VAM_QUIE]);
	send_to_char(buf, ch);
	sprintf(buf,
		"Thaumaturgy:  [%li]             Auspex:     [%li]        Dominate:    [%li]\n\r",
		ch->pcdata->powers[VAM_THAU], ch->pcdata->powers[VAM_AUSP], ch->pcdata->powers[VAM_DOMI]);
	send_to_char(buf, ch);
	sprintf(buf,
		"Obfuscate:    [%li]             Potence:    [%li]        Protean:     [%li]\n\r",
		ch->pcdata->powers[VAM_OBFU], ch->pcdata->powers[VAM_POTE], ch->pcdata->powers[VAM_PROT]);
	send_to_char(buf, ch);
	sprintf(buf,
		"Serpentis:    [%li]             Vicissitude:[%li]        Daimoinon:   [%li]\n\r",
		ch->pcdata->powers[VAM_SERP], ch->pcdata->powers[VAM_VICI], ch->pcdata->powers[VAM_DAIM]);
	send_to_char(buf, ch);
	sprintf(buf,
		"Chimerstry:   [%li]             Obeah:      [%li]        Melpominee:  [%li]\n\r",
		ch->pcdata->powers[VAM_CHIM], ch->pcdata->powers[VAM_OBEA], ch->pcdata->powers[VAM_MELP]);
	send_to_char(buf, ch);
	sprintf(buf,
		"Necromancy:   [%li]                                    Thanatosis:  [%li]\n\r",
		ch->pcdata->powers[VAM_NECR], ch->pcdata->powers[VAM_THAN]);
	stc(buf, ch);
	send_to_char(lin, ch);
...
*/
		...
		if(cost > ch->practice)
		{
			sprintf(buf, "It costs you %li primal to improve your %s discipline.\n\r", cost, arg1);
			send_to_char(buf, ch);
			return;
		}
		...
		send_to_char("To improve a discipline, type: Clandisc <discipline type> improve.\n\r", ch);
	return;
}
...
void do_oldclandisc(CHAR_DATA * ch, char *argument)
{
	...
	if(arg[0] == '\0')
	{
		...
		{
			sprintf(buf, "It will cost you %li primal to gain another discipline.\n\r", cost);
			send_to_char(buf, ch);
		}
		if(cost <= ch->practice)
			send_to_char("Select from:", ch);
		else
			return;
	...
	if(clancount >= clanmax)
	{
		if(!str_cmp(arg, "protean") && (IS_VAMPAFF(ch, VAM_PROTEAN) || IS_VAMPPASS(ch, VAM_PROTEAN)))
			...
			&& (IS_VAMPAFF(ch, VAM_VICISSITUDE) || IS_VAMPPASS(ch, VAM_VICISSITUDE)))
			...
			&& (IS_VAMPAFF(ch, VAM_OBTENEBRATION) || IS_VAMPPASS(ch, VAM_OBTENEBRATION)))
	...
	{
		if(IS_VAMPAFF(ch, VAM_PROTEAN) || IS_VAMPPASS(ch, VAM_PROTEAN) || cost > ch->practice)
		{
			send_to_char("Powers: Nightsight, Claws, Change.\n\r", ch);
			return;
		}
...
*/
		if(clancount < 3)
			SET_BIT(ch->pcdata->stats[UNI_CURRENT], VAM_PROTEAN);
	...
	{
		if(IS_VAMPAFF(ch, VAM_CELERITY) || IS_VAMPPASS(ch, VAM_CELERITY) || cost > ch->practice)
		{
			send_to_char("Powers: An extra attack, Reduced move cost for spells.\n\r", ch);
			return;
		}
		...
		if(clancount < 3)
			SET_BIT(ch->pcdata->stats[UNI_CURRENT], VAM_CELERITY);
/*
...
*/
	...
	{
		if(IS_VAMPAFF(ch, VAM_FORTITUDE) || IS_VAMPPASS(ch, VAM_FORTITUDE) || cost > ch->practice)
		{
			send_to_char("Powers: 1-100% damage reduction, +50 one time hp bonus.\n\r", ch);
			return;
		}
		...
		if(clancount < 3)
			SET_BIT(ch->pcdata->stats[UNI_CURRENT], VAM_FORTITUDE);
/*
...
*/
	...
	{
		if(IS_VAMPAFF(ch, VAM_POTENCE) || IS_VAMPPASS(ch, VAM_POTENCE) || cost > ch->practice)
		{
			send_to_char("Powers: 150% normal damage in combat.\n\r", ch);
			return;
		}
...
*/
		if(clancount < 3)
			SET_BIT(ch->pcdata->stats[UNI_CURRENT], VAM_POTENCE);
	...
	{
		if(IS_VAMPAFF(ch, VAM_OBFUSCATE) || IS_VAMPPASS(ch, VAM_OBFUSCATE) || cost > ch->practice)
		{
			send_to_char("Powers: Mask, Mortal, Shield.\n\r", ch);
			return;
		}
		...
		if(clancount < 3)
			SET_BIT(ch->pcdata->stats[UNI_CURRENT], VAM_OBFUSCATE);
/*
...
*/
	...
	{
		if(IS_VAMPAFF(ch, VAM_OBTENEBRATION) || IS_VAMPPASS(ch, VAM_OBTENEBRATION) || cost > ch->practice)
		{
			send_to_char("Powers: Shadowplane, Shadowsight, Nightsight.\n\r", ch);
			return;
		}
		...
		if(clancount < 3)
			SET_BIT(ch->pcdata->stats[UNI_CURRENT], VAM_OBTENEBRATION);
/*
...
*/
	...
	{
		if(IS_VAMPAFF(ch, VAM_SERPENTIS) || IS_VAMPPASS(ch, VAM_SERPENTIS) || cost > ch->practice)
		{
			send_to_char("Powers: Darkheart, Serpent, Poison, Nightsight.\n\r", ch);
			return;
		}
		...
		if(clancount < 3)
			SET_BIT(ch->pcdata->stats[UNI_CURRENT], VAM_SERPENTIS);
/*
...
*/
	...
	{
		if(IS_VAMPAFF(ch, VAM_VICISSITUDE) || IS_VAMPPASS(ch, VAM_VICISSITUDE) || cost > ch->practice)
		{
		...
		if(clancount < 3)
			SET_BIT(ch->pcdata->stats[UNI_CURRENT], VAM_VICISSITUDE);
/*
...
*/
	...
	{
		if(IS_VAMPAFF(ch, VAM_AUSPEX) || IS_VAMPPASS(ch, VAM_AUSPEX) || cost > ch->practice)
		{
			send_to_char("Powers: Truesight, Scry, Readaura.\n\r", ch);
			return;
		}
		...
		if(clancount < 3)
			SET_BIT(ch->pcdata->stats[UNI_CURRENT], VAM_AUSPEX);
/*
...
*/
	...
	{
		if(IS_VAMPAFF(ch, VAM_DOMINATE) || IS_VAMPPASS(ch, VAM_DOMINATE) || cost > ch->practice)
		{
			send_to_char("Powers: Evileye, Command, Shield, Ghoul.\n\r", ch);
			return;
		}
		...
		if(clancount < 3)
			SET_BIT(ch->pcdata->stats[UNI_CURRENT], VAM_DOMINATE);
/*
...
*/
	...
	{
		if(IS_VAMPAFF(ch, VAM_PRESENCE) || IS_VAMPPASS(ch, VAM_PRESENCE) || cost > ch->practice)
		{
			send_to_char("Powers: Majesty.\n\r", ch);
			return;
		}
		...
		if(clancount < 3)
			SET_BIT(ch->pcdata->stats[UNI_CURRENT], VAM_PRESENCE);
...
void do_shadowplane(CHAR_DATA * ch, char *argument)
{
	...
	if(!IS_CLASS(ch, CLASS_WEREWOLF) || ch->pcdata->powers[WPOWER_OWL] < 3)
	{
		...
		if(!IS_CLASS(ch, CLASS_VAMPIRE) || ch->pcdata->powers[VAM_OBTE] < 3)
		{
	...
	if(arg[0] == '\0')
	{
		if(!IS_AFFECTED(ch, AFF_SHADOWPLANE))
		{
			...
			act("The shadows flicker and swallow up $n.", ch, 0, 0, TO_ROOM);
		...
		act("The shadows flicker and $n fades into existance.", ch, 0, 0, TO_ROOM);
		do_look(ch, "auto");
		return;
	}
...
void do_bloodline(CHAR_DATA * ch, char *argument)
{
	...
	if(arg3[0] != '\0')
	{
		sprintf(buf, "My name is %s, of the %s tribe, pup of %s.", arg3, ch->clan, arg2);
		do_say(ch, buf);
	}
	if(arg2[0] != '\0')
	{
...
*/
	...
	if(ch->pcdata->stats[UNI_GEN] == 1)
		...
		sprintf(buf, "As is the tradition, I recite the lineage of %s, %s of the %s Generation.", ch->name, buf3,
			buf2);
	do_say(ch, buf);
	if(ch->pcdata->stats[UNI_GEN] != 1)
	{
	...
	if(ch->pcdata->stats[UNI_GEN] == 4)
		sprintf(buf, "My name is %s, childe of %s, childe of %s, childe of %s.  Recognize my lineage.", ch->name,
			arg3, arg2, arg1);
	if(ch->pcdata->stats[UNI_GEN] == 3)
		sprintf(buf, "My name is %s, childe of %s, childe of %s.  Recognize my lineage.", ch->name, arg2, arg1);
	if(ch->pcdata->stats[UNI_GEN] == 2)
		sprintf(buf, "My name is %s, childe of %s.  Recognize my lineage.", ch->name, arg1);
	if(ch->pcdata->stats[UNI_GEN] == 1)
...
void do_darkheart(CHAR_DATA * ch, char *argument)
{
	...
	act("$n rips $s heart out and tosses it to the ground.", ch, 0, 0, TO_ROOM);
	make_part(ch, "heart");
...
void do_truesight(CHAR_DATA * ch, char *argument)
{
	...
	else if(!IS_CLASS(ch, CLASS_WEREWOLF) || ch->pcdata->powers[WPOWER_HAWK] < 3)
	{
		...
		if(IS_CLASS(ch, CLASS_VAMPIRE) && ch->pcdata->powers[VAM_AUSP] < 1)
		{
...
void do_scry(CHAR_DATA * ch, char *argument)
{
	...
	if(!IS_CLASS(ch, CLASS_DEMON) && !IS_SET(ch->special, SPC_CHAMPION) &&
	   !IS_CLASS(ch, CLASS_VAMPIRE) && ch->pcdata->powers[VAM_AUSP] < 2 &&
	   !IS_CLASS(ch, CLASS_NINJA) && !IS_CLASS(ch, CLASS_MONK))
	{
		send_to_char("You need level 2 auspex first.\n\r", ch);
		return;
	}
	...
	{
		act("Your spirit guardian sends you a message:", ch, 0, victim, TO_VICT);
		act("\"$n is scrying you.\"", ch, 0, victim, TO_VICT);
	}
...
void do_readaura(CHAR_DATA * ch, char *argument)
{
	...
	if(IS_CLASS(ch, CLASS_VAMPIRE) && ch->pcdata->powers[VAM_AUSP] <
	   5 && !IS_ITEMAFF(ch, ITEMA_VISION) && !IS_CLASS(ch, CLASS_NINJA) && !IS_CLASS(ch, CLASS_MONK))
	{
	...
	if((victim = get_char_room(ch, arg)) == 0)
	{
		...
		if(!IS_ITEMAFF(ch, ITEMA_VISION))
			ch->pcdata->condition[COND_THIRST] -= number_range(40, 50);
		act("$n examines $p intently.", ch, obj, 0, TO_ROOM);
	...
	if(!IS_ITEMAFF(ch, ITEMA_VISION))
		ch->pcdata->condition[COND_THIRST] -= number_range(40, 50);
	if(!IS_NPC(victim) && (IS_IMMUNE(victim, IMM_SHIELDED) || (IS_CLASS(victim, CLASS_DROW) &&
								   IS_SET(victim->pcdata->powers[1], DPOWER_DROWSHIELD)))
	   && !IS_ITEMAFF(ch, ITEMA_VISION))
	{
	...
	act("$n examines $N intently.", ch, 0, victim, TO_NOTVICT);
	act("$n examines you intently.", ch, 0, victim, TO_VICT);
	if(IS_NPC(victim))
	...
	if(!IS_NPC(victim))
	{
		sprintf(buf, "Status:%li, ", victim->pcdata->status);
		send_to_char(buf, ch);
		if(IS_CLASS(victim, CLASS_VAMPIRE))
		{
			sprintf(buf, "Blood:%li, ", victim->pcdata->condition[COND_THIRST]);
			send_to_char(buf, ch);
		}
	}
	sprintf(buf, "Alignment:%li.\n\r", victim->alignment);
	send_to_char(buf, ch);
	if(!IS_NPC(victim) && IS_EXTRA(victim, EXTRA_PREGNANT))
		act("$N is pregnant.", ch, 0, victim, TO_CHAR);
...
void do_mortal(CHAR_DATA * ch, char *argument)
{
	...
	if(IS_CLASS(ch, CLASS_VAMPIRE))
	{
		...
		act("Colour returns to $n's skin.", ch, 0, 0, TO_ROOM);

		REMOVE_BIT(ch->pcdata->class, CLASS_VAMPIRE);
		SET_BIT(ch->pcdata->stats[UNI_AFF], VAM_MORTAL);
		return;
	}
	send_to_char("You skin pales and cools.\n\r", ch);
	act("$n's skin pales slightly.", ch, 0, 0, TO_ROOM);
	ch->pcdata->class = CLASS_VAMPIRE;
...
void do_shield(CHAR_DATA * ch, char *argument)
{
	...
	if((!IS_CLASS(ch, CLASS_VAMPIRE) || ch->pcdata->powers[VAM_OBFU] < 2) && !IS_CLASS(ch, CLASS_DROW)
	   && !IS_CLASS(ch, CLASS_WEREWOLF) && !IS_CLASS(ch, CLASS_MONK) && !IS_CLASS(ch, CLASS_DEMON))
	{
...
void do_serpent(CHAR_DATA * ch, char *argument)
{
	...
	if(IS_AFFECTED(ch, AFF_POLYMORPH))
	{
		...
		act("You transform back into human.", ch, 0, 0, TO_CHAR);
		act("$n transform into human form.", ch, 0, 0, TO_ROOM);
	...
	if(ch->wpn[0] > 0)
	{
		ch->hitroll += (long) (ch->wpn[0] * 0.75);
		ch->damroll += (long) (ch->wpn[0] * 0.75);
		ch->armor -= (long) (ch->wpn[0] * 3);
	}
	if(ch->stance[0] != -1)
		do_stance(ch, "");
	if(ch->mounted == IS_RIDING)
		do_dismount(ch, "");
	ch->pcdata->mod_str = 10;
	act("You transform into a huge serpent.", ch, 0, 0, TO_CHAR);
	act("$n transforms into a huge serpent.", ch, 0, 0, TO_ROOM);
...
void do_regenerate(CHAR_DATA * ch, char *argument)
{
	...
	if(ch->hit >= ch->max_hit && ch->mana >= ch->max_mana && ch->move >= ch->max_move)
	{
		...
		act("$n's body completely regenerates itself.", ch, 0, 0, TO_ROOM);
	}
	else
	...
	{
		min = 5 - ch->pcdata->stats[UNI_GEN];
		max = 25 - (ch->pcdata->stats[UNI_GEN] * 2);
...
void reg_mend(CHAR_DATA * ch)
{
	...
	if(ribs > 0)
	{
		...
		act("One of $n's ribs snap back into place.", ch, 0, 0, TO_ROOM);
		act("One of your ribs snap back into place.", ch, 0, 0, TO_CHAR);
	}
	else if(IS_HEAD(ch, LOST_EYE_L))
	{
		act("An eyeball appears in $n's left eye socket.", ch, 0, 0, TO_ROOM);
		act("An eyeball appears in your left eye socket.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_HEAD], LOST_EYE_L);
	}
	else if(IS_HEAD(ch, LOST_EYE_R))
	{
		act("An eyeball appears in $n's right eye socket.", ch, 0, 0, TO_ROOM);
		act("An eyeball appears in your right eye socket.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_HEAD], LOST_EYE_R);
	}
	else if(IS_HEAD(ch, LOST_EAR_L))
	{
		act("An ear grows on the left side of $n's head.", ch, 0, 0, TO_ROOM);
		act("An ear grows on the left side of your head.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_HEAD], LOST_EAR_L);
	}
	else if(IS_HEAD(ch, LOST_EAR_R))
	{
		act("An ear grows on the right side of $n's head.", ch, 0, 0, TO_ROOM);
		act("An ear grows on the right side of your head.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_HEAD], LOST_EAR_R);
	}
	else if(IS_HEAD(ch, LOST_NOSE))
	{
		act("A nose grows on the front of $n's face.", ch, 0, 0, TO_ROOM);
		act("A nose grows on the front of your face.", ch, 0, 0, TO_CHAR);
	...
	else if(teeth > 0)
	{
		...
		act("A missing tooth grows in your mouth.", ch, 0, 0, TO_CHAR);
		act("A missing tooth grows in $n's mouth.", ch, 0, 0, TO_ROOM);
	}
	else if(IS_HEAD(ch, BROKEN_NOSE))
	{
		act("$n's nose snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your nose snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_HEAD], BROKEN_NOSE);
	}
	else if(IS_HEAD(ch, BROKEN_JAW))
	{
		act("$n's jaw snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your jaw snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_HEAD], BROKEN_JAW);
	}
	else if(IS_HEAD(ch, BROKEN_SKULL))
	{
		act("$n's skull knits itself back together.", ch, 0, 0, TO_ROOM);
		act("Your skull knits itself back together.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_HEAD], BROKEN_SKULL);
	}
	else if(IS_BODY(ch, BROKEN_SPINE))
	{
		act("$n's spine knits itself back together.", ch, 0, 0, TO_ROOM);
		act("Your spine knits itself back together.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_BODY], BROKEN_SPINE);
	}
	else if(IS_BODY(ch, BROKEN_NECK))
	{
		act("$n's neck snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your neck snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_BODY], BROKEN_NECK);
	}
	else if(IS_ARM_L(ch, LOST_ARM))
	{
		act("An arm grows from the stump of $n's left shoulder.", ch, 0, 0, TO_ROOM);
		act("An arm grows from the stump of your left shoulder.", ch, 0, 0, TO_CHAR);
	...
	{
		act("An arm grows from the stump of $n's right shoulder.", ch, 0, 0, TO_ROOM);
		act("An arm grows from the stump of your right shoulder.", ch, 0, 0, TO_CHAR);
	...
	{
		act("A leg grows from the stump of $n's left hip.", ch, 0, 0, TO_ROOM);
		act("A leg grows from the stump of your left hip.", ch, 0, 0, TO_CHAR);
	...
	{
		act("A leg grows from the stump of $n's right hip.", ch, 0, 0, TO_ROOM);
		act("A leg grows from the stump of your right hip.", ch, 0, 0, TO_CHAR);
	...
	{
		act("$n's left arm snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your left arm snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_L], BROKEN_ARM);
	}
	else if(IS_ARM_R(ch, BROKEN_ARM))
	{
		act("$n's right arm snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your right arm snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_R], BROKEN_ARM);
	}
	else if(IS_LEG_L(ch, BROKEN_LEG))
	{
		act("$n's left leg snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your left leg snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_LEG_L], BROKEN_LEG);
	}
	else if(IS_LEG_R(ch, BROKEN_LEG))
	{
		act("$n's right leg snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your right leg snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_LEG_R], BROKEN_LEG);
	}
	else if(IS_ARM_L(ch, LOST_HAND))
	{
		act("A hand grows from the stump of $n's left wrist.", ch, 0, 0, TO_ROOM);
		act("A hand grows from the stump of your left wrist.", ch, 0, 0, TO_CHAR);
	...
	{
		act("A hand grows from the stump of $n's right wrist.", ch, 0, 0, TO_ROOM);
		act("A hand grows from the stump of your right wrist.", ch, 0, 0, TO_CHAR);
	...
	{
		act("A foot grows from the stump of $n's left ankle.", ch, 0, 0, TO_ROOM);
		act("A foot grows from the stump of your left ankle.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_LEG_L], LOST_FOOT);
	}
	else if(IS_LEG_R(ch, LOST_FOOT))
	{
		act("A foot grows from the stump of $n's right ankle.", ch, 0, 0, TO_ROOM);
		act("A foot grows from the stump of your right ankle.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_LEG_R], LOST_FOOT);
	}
	else if(IS_ARM_L(ch, LOST_THUMB))
	{
		act("A thumb slides out of $n's left hand.", ch, 0, 0, TO_ROOM);
		act("A thumb slides out of your left hand.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_L], LOST_THUMB);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_L], BROKEN_THUMB);
	}
	else if(IS_ARM_L(ch, BROKEN_THUMB))
	{
		act("$n's left thumb snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your left thumb snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_L], BROKEN_THUMB);
	}
	else if(IS_ARM_L(ch, LOST_FINGER_I))
	{
		act("An index finger slides out of $n's left hand.", ch, 0, 0, TO_ROOM);
		act("An index finger slides out of your left hand.", ch, 0, 0, TO_CHAR);
	...
	{
		act("$n's left index finger snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your left index finger snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_L], BROKEN_FINGER_I);
	}
	else if(IS_ARM_L(ch, LOST_FINGER_M))
	{
		act("A middle finger slides out of $n's left hand.", ch, 0, 0, TO_ROOM);
		act("A middle finger slides out of your left hand.", ch, 0, 0, TO_CHAR);
	...
	{
		act("$n's left middle finger snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your left middle finger snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_L], BROKEN_FINGER_M);
	}
	else if(IS_ARM_L(ch, LOST_FINGER_R))
	{
		act("A ring finger slides out of $n's left hand.", ch, 0, 0, TO_ROOM);
		act("A ring finger slides out of your left hand.", ch, 0, 0, TO_CHAR);
	...
	{
		act("$n's left ring finger snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your left ring finger snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_L], BROKEN_FINGER_R);
	}
	else if(IS_ARM_L(ch, LOST_FINGER_L))
	{
		act("A little finger slides out of $n's left hand.", ch, 0, 0, TO_ROOM);
		act("A little finger slides out of your left hand.", ch, 0, 0, TO_CHAR);
	...
	{
		act("$n's left little finger snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your left little finger snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_L], BROKEN_FINGER_L);
	}
	else if(IS_ARM_R(ch, LOST_THUMB))
	{
		act("A thumb slides out of $n's right hand.", ch, 0, 0, TO_ROOM);
		act("A thumb slides out of your right hand.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_R], LOST_THUMB);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_R], BROKEN_THUMB);
	}
	else if(IS_ARM_R(ch, BROKEN_THUMB))
	{
		act("$n's right thumb snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your right thumb snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_R], BROKEN_THUMB);
	}
	else if(IS_ARM_R(ch, LOST_FINGER_I))
	{
		act("An index finger slides out of $n's right hand.", ch, 0, 0, TO_ROOM);
		act("An index finger slides out of your right hand.", ch, 0, 0, TO_CHAR);
	...
	{
		act("$n's right index finger snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your right index finger snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_R], BROKEN_FINGER_I);
	}
	else if(IS_ARM_R(ch, LOST_FINGER_M))
	{
		act("A middle finger slides out of $n's right hand.", ch, 0, 0, TO_ROOM);
		act("A middle finger slides out of your right hand.", ch, 0, 0, TO_CHAR);
	...
	{
		act("$n's right middle finger snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your right middle finger snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_R], BROKEN_FINGER_M);
	}
	else if(IS_ARM_R(ch, LOST_FINGER_R))
	{
		act("A ring finger slides out of $n's right hand.", ch, 0, 0, TO_ROOM);
		act("A ring finger slides out of your right hand.", ch, 0, 0, TO_CHAR);
	...
	{
		act("$n's right ring finger snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your right ring finger snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_R], BROKEN_FINGER_R);
	}
	else if(IS_ARM_R(ch, LOST_FINGER_L))
	{
		act("A little finger slides out of $n's right hand.", ch, 0, 0, TO_ROOM);
		act("A little finger slides out of your right hand.", ch, 0, 0, TO_CHAR);
	...
	{
		act("$n's right little finger snaps back into place.", ch, 0, 0, TO_ROOM);
		act("Your right little finger snaps back into place.", ch, 0, 0, TO_CHAR);
		REMOVE_BIT(ch->loc_hp[LOC_ARM_R], BROKEN_FINGER_L);
	}
	else if(IS_BODY(ch, CUT_THROAT))
	{
		if(IS_SET(ch->loc_hp[6], BLEEDING_THROAT))
			return;
		act("The wound in $n's throat closes up.", ch, 0, 0, TO_ROOM);
		act("The wound in your throat closes up.", ch, 0, 0, TO_CHAR);
...
void vamp_rage(CHAR_DATA * ch)
{
	...
	act("$n screams with rage as $s inner beast consumes $m!.", ch, 0, 0, TO_ROOM);
	do_beastlike(ch, "");
	do_rage(ch, "");
	return;
}
	...
	if(IS_CLASS(ch, CLASS_DEMON) || IS_SET(ch->special, SPC_CHAMPION))
	{
		...
		if(IS_DEMPOWER(ch, DEM_HORNS))
		{
			if(IS_DEMAFF(ch, DEM_HORNS))
				...
				send_to_char("You have a pair of curved horns, but they are not currently extended.\n\r",
					     ch);
		}
		if(IS_DEMPOWER(ch, DEM_TAIL))
		{
			if(IS_DEMAFF(ch, DEM_TAIL))
				send_to_char("You have a long tail extended from you back side.\n\r", ch);
			else
		...
		if(IS_DEMPOWER(ch, DEM_HOOVES))
		{
			if(IS_DEMAFF(ch, DEM_HOOVES))
				...
				send_to_char("You are able to transform your feet into hooves at will.\n\r", ch);
		}
		if(IS_DEMPOWER(ch, DEM_EYES))
		{
			if(IS_VAMPAFF(ch, VAM_NIGHTSIGHT))
		...
		if(IS_DEMPOWER(ch, DEM_WINGS))
		{
			if(!IS_DEMAFF(ch, DEM_WINGS))
				...
				send_to_char("You have a pair of large leathery wings folded behind your back.\n\r", ch);
		}
...
/* Paladin Upkeep of powers - Loki */
	...
	else if(IS_CLASS(ch, CLASS_VAMPIRE))
	{
		...
		if(ch->pcdata->powers[VAM_PROT] > 8)
...
void do_vclan(CHAR_DATA * ch, char *argument)
{
	...
	for(gch = char_list; gch != 0; gch = gch->next)
	{
		...
		{
			sprintf(buf,
				"[%-16s] [  %li  ] [%-6li%3li] [%-6li%3li] [%-6li%3li] [%7li] [ %3li ]\n\r",
				capitalize(gch->name),
...
void do_tribe(CHAR_DATA * ch, char *argument)
{
	...
	for(gch = char_list; gch != 0; gch = gch->next)
	{
		...
		if(!IS_CLASS(gch, CLASS_WEREWOLF))
			continue;
		if(strlen(gch->clan) > 16)
			continue;
		if(gch->clan != 0 && strlen(gch->clan) > 1)
		...
		{
			sprintf(buf,
				"[%-16s] [%-13s] [%-6li%3li] [%-6li%3li] [%-6li%3li] [%7li]\n\r",
...
void do_werewolf(CHAR_DATA * ch, char *argument)
{
	...
	act("$n throws back $s head and howls with rage!.", ch, 0, 0, TO_ROOM);
	send_to_char("Coarse dark hair sprouts from your body.\n\r", ch);
	act("Coarse dark hair sprouts from $n's body.", ch, 0, 0, TO_ROOM);
	if(!IS_VAMPAFF(ch, VAM_NIGHTSIGHT))
	{
		send_to_char("Your eyes start glowing red.\n\r", ch);
		act("$n's eyes start glowing red.", ch, 0, 0, TO_ROOM);
	...
	if(!IS_VAMPAFF(ch, VAM_FANGS))
	{
		...
		act("A pair of long fangs extend from $n's mouth.", ch, 0, 0, TO_ROOM);
		SET_BIT(ch->pcdata->stats[UNI_AFF], VAM_FANGS);
	}
	if(!IS_VAMPAFF(ch, VAM_CLAWS))
	{
	...
	if((obj = get_eq_char(ch, WEAR_WIELD)) != 0 && !IS_SET(obj->spectype, SITEM_WOLFWEAPON))
	{
		act("$p drops from your right hand.", ch, obj, 0, TO_CHAR);
		act("$p drops from $n's right hand.", ch, obj, 0, TO_ROOM);
		obj_from_char(obj);
		obj_to_char(obj, ch);
	}
	if((obj = get_eq_char(ch, WEAR_HOLD)) != 0 && !IS_SET(obj->spectype, SITEM_WOLFWEAPON))
	{
		act("$p drops from your left hand.", ch, obj, 0, TO_CHAR);
		act("$p drops from $n's left hand.", ch, obj, 0, TO_ROOM);
		obj_from_char(obj);
		obj_to_char(obj, ch);
	}
	SET_BIT(ch->affected_by, AFF_POLYMORPH);
	...
	for(vch = char_list; vch != 0; vch = vch_next)
	{
		...
		if(ch == vch)
		{
			act("You throw back your head and howl with rage!", ch, 0, 0, TO_CHAR);
			continue;
		}
		...
		if(!IS_NPC(vch))
		{
			if(vch->in_room == ch->in_room)
				act("$n throws back $s head and howls with rage!", ch, 0, vch, TO_VICT);
...
/*		send_to_char("You hear a fearsome howl far off in the
   distance!\n\r", vch);*/
		...
		if(vch->in_room == ch->in_room && can_see(ch, vch))
		{
			...
			if(vch == 0 || vch->position <= POS_STUNNED)
				continue;
			multi_hit(ch, vch, TYPE_UNDEFINED);
			if(vch == 0 || vch->position <= POS_STUNNED)
				continue;
...
void do_unwerewolf(CHAR_DATA * ch, char *argument)
{
	...
	if(IS_VAMPAFF(ch, VAM_FANGS))
	{
		...
		act("$n's fangs slide back into $s mouth.", ch, 0, 0, TO_ROOM);
		REMOVE_BIT(ch->pcdata->stats[UNI_AFF], VAM_FANGS);
	}
	if(IS_VAMPAFF(ch, VAM_NIGHTSIGHT))
	{
	...
	act("$n's coarse hair shrinks back into $s body.", ch, 0, 0, TO_ROOM);
	ch->pcdata->stats[UNI_RAGE] -= 75;
	if(ch->pcdata->stats[UNI_RAGE] < 0)
		ch->pcdata->stats[UNI_RAGE] = 0;
	return;
...
void do_favour(CHAR_DATA * ch, char *argument)
{
	...
	if(!str_cmp(arg2, "prince") && ch->pcdata->stats[UNI_GEN] == 2)
	{
		...
		{
			act("You remove $N's prince privilages!", ch, 0, victim, TO_CHAR);
			act("$n removes $N's prince privilages!", ch, 0, victim, TO_NOTVICT);
			act("$n removes your prince privilages!", ch, 0, victim, TO_VICT);
			if(IS_SET(victim->special, SPC_SIRE))
				REMOVE_BIT(victim->special, SPC_SIRE);
			REMOVE_BIT(victim->special, SPC_PRINCE);
			return;
		}
		act("You make $N a prince!", ch, 0, victim, TO_CHAR);
		act("$n has made $N a prince!", ch, 0, victim, TO_NOTVICT);
		act("$n has made you a prince!", ch, 0, victim, TO_VICT);
	...
	else if(!str_cmp(arg2, "sire") && (ch->pcdata->stats[UNI_GEN] == 2 || IS_SET(ch->special, SPC_PRINCE)))
	{
		if(IS_SET(victim->special, SPC_SIRE))
		{
			act("You remove $N's permission to sire a childe!", ch, 0, victim, TO_CHAR);
			act("$n has removed $N's permission to sire a childe!", ch, 0, victim, TO_NOTVICT);
			act("$n has remove your permission to sire a childe!", ch, 0, victim, TO_VICT);
			REMOVE_BIT(victim->special, SPC_SIRE);
			return;
		}
		act("You grant $N permission to sire a childe!", ch, 0, victim, TO_CHAR);
		act("$n has granted $N permission to sire a childe!", ch, 0, victim, TO_NOTVICT);
		act("$n has granted you permission to sire a childe!", ch, 0, victim, TO_VICT);
		SET_BIT(victim->special, SPC_SIRE);
		return;
	}
	else if(!str_cmp(arg2, "outcast") && victim->pcdata->stats[UNI_GEN] > 2 && ch->pcdata->stats[UNI_GEN] == 2)
	{
		act("You make $N a Caitiff!", ch, 0, victim, TO_CHAR);
		act("$n has made $N a Caitiff!", ch, 0, victim, TO_NOTVICT);
		act("$n has made you a Caitiff!", ch, 0, victim, TO_VICT);
		free_string(victim->clan);
		victim->clan = str_dup("");
		return;
	}
		...
		ch->pcdata->stats[UNI_GEN] == 2 && strlen(victim->clan) < 2)
	{
		...
		act("You induct $N into your clan!", ch, 0, victim, TO_CHAR);
		act("$n inducts $N into $s clan!", ch, 0, victim, TO_NOTVICT);
		act("$n inducts you into $s clan!", ch, 0, victim, TO_VICT);
		free_string(victim->clan);
		victim->clan = str_dup(ch->clan);
		return;
	}
	else if(!str_cmp(arg2, "induct") && victim->pcdata->stats[UNI_GEN] > 2 &&
		!IS_SET(victim->special, SPC_PRINCE) && IS_SET(ch->special, SPC_PRINCE) && strlen(victim->clan) < 2)
	{
		if(IS_SET(victim->special, SPC_ANARCH))
		{
			send_to_char("You cannot induct an Anarch!\n\r", ch);
	...
	else if(!str_cmp(arg2, "accept") && (ch->pcdata->stats[UNI_GEN] == 2 || IS_SET(ch->special, SPC_PRINCE)))
	{
		...
		act("You accept $N into the clan!", ch, 0, victim, TO_CHAR);
		act("$n has accepted $N into $s clan!", ch, 0, victim, TO_NOTVICT);
		act("$n accepted you into $s clan!", ch, 0, victim, TO_VICT);
...
*/
	...
	victim->wizard = ch;
	act("You cut open your wrist and feed $N some blood.", ch, 0, victim, TO_CHAR);
	act("$n cuts open $s wrist and feeds you some blood.", ch, 0, victim, TO_VICT);
	act("$n cuts open $s wrist and feeds $N some blood.", ch, 0, victim, TO_NOTVICT);
	return;
}
...
*/
	if((victim = ch->pcdata->familiar) == 0)
	{
...
void do_vampvanish(CHAR_DATA * ch, char *argument)
{
	if(IS_NPC(ch))
		return;
	...
	if(IS_CLASS(ch, CLASS_VAMPIRE) && ch->pcdata->powers[VAM_OBFU] < 1)
	{
...
{
	act("You flex your bulging muscles.", ch, 0, 0, TO_CHAR);
	act("$n flexes $s bulging muscles.", ch, 0, 0, TO_ROOM);
	if(IS_NPC(ch))
		return;

	if(IS_EXTRA(ch, TIED_UP))
	{
		act("The ropes restraining you snap.", ch, 0, 0, TO_CHAR);
		act("The ropes restraining $n snap.", ch, 0, 0, TO_ROOM);
		REMOVE_BIT(ch->extra, TIED_UP);
	}
	if(is_affected(ch, gsn_web))
	{
		act("The webbing entrapping $n breaks away.", ch, 0, 0, TO_ROOM);
...
void do_rage(CHAR_DATA * ch, char *argument)
{
	...
	if(!IS_SET(ch->special, SPC_WOLFMAN))
	{
		...
		act("$n starts snarling angrilly.", ch, 0, 0, TO_ROOM);
		if(ch->pcdata->stats[UNI_GEN] < 3)
			ch->pcdata->stats[UNI_RAGE] += number_range(40, 70);
		else
			ch->pcdata->stats[UNI_RAGE] += number_range(10, 20);
...
void do_calm(CHAR_DATA * ch, char *argument)
{
	...
	if(IS_CLASS(ch, CLASS_VAMPIRE) && ch->beast < 1)
	{
		...
		act("$n takes a deep breath and forces back $s inner beast.", ch, 0, 0, TO_ROOM);
		ch->pcdata->stats[UNI_RAGE] = 0;
		if(IS_VAMPAFF(ch, VAM_NIGHTSIGHT))
			do_nightsight(ch, "");
		if(IS_VAMPAFF(ch, VAM_FANGS))
	...
	if(IS_SET(ch->special, SPC_WOLFMAN))
	{
		...
		act("$n takes a deep breath and tries to calm $mself.", ch, 0, 0, TO_ROOM);
		if(ch->pcdata->stats[UNI_GEN] < 3)
			ch->pcdata->stats[UNI_RAGE] -= number_range(100, 170);
		else
			ch->pcdata->stats[UNI_RAGE] -= number_range(10, 20);
		if(ch->pcdata->stats[UNI_RAGE] < 100)
			do_unwerewolf(ch, "");
...
void do_totems(CHAR_DATA * ch, char *argument)
{
	...
	{
		sprintf(buf,
			"Totems: Bear (%li), Tiger (%li), Lynx (%li), Gaia (%li), Boar (%li), Owl (%li),\n\r Spider (%li), Wolf (%li), Hawk (%li), Mantis (%li).\n\r",
			ch->pcdata->powers[WPOWER_BEAR], ch->pcdata->powers[WPOWER_TIGER], ch->pcdata->powers[WPOWER_LYNX],
			ch->pcdata->powers[WPOWER_GAIA], ch->pcdata->powers[WPOWER_BOAR], ch->pcdata->powers[WPOWER_OWL],
			ch->pcdata->powers[WPOWER_SPIDER], ch->pcdata->powers[WPOWER_WOLF], ch->pcdata->powers[WPOWER_HAWK],
	...
	if(arg2[0] == '\0')
	{
		...
		else if(!str_cmp(arg1, "Spider"))
		{
			...
			if(ch->pcdata->powers[WPOWER_SPIDER] > 3)
				send_to_char("BASH: Send your opponents sprawling with a powerful bash.\n\r", ch);
			return;
		}
		...
		else if(!str_cmp(arg1, "Mantis"))
		{
			...
			if(ch->pcdata->powers[WPOWER_MANTIS] > 3)
				send_to_char
					("Incredibly fast attacks: Your opponents get -20 to parry and -40 to dodge.\n\r",
					 ch);
			else if(ch->pcdata->powers[WPOWER_MANTIS] > 2)
				send_to_char
					("Extremely fast attacks: Your opponents get -15 to parry and -30 to dodge.\n\r",
					 ch);
		...
		sprintf(buf,
			"Totems: Bear (%li), Lynx (%li), Boar (%li), Owl (%li), Spider (%li), Wolf (%li),\n\r        Hawk (%li), Mantis (%li).\n\r",
			ch->pcdata->powers[WPOWER_BEAR], ch->pcdata->powers[WPOWER_LYNX], ch->pcdata->powers[WPOWER_BOAR],
			ch->pcdata->powers[WPOWER_OWL], ch->pcdata->powers[WPOWER_SPIDER], ch->pcdata->powers[WPOWER_WOLF],
	...
	if(!str_cmp(arg2, "improve"))
	{
		...
		{
			improve = WPOWER_MANTIS;
			max = 4;
		}
		else
		{
			send_to_char
				("You can improve: Bear, Tiger, Boar, Gaia, Lynx, Owl, Spider, Wolf, Hawk or Mantis.\n\r",
				 ch);
			return;
		}
		cost = (ch->pcdata->powers[improve] + 1) * 10;
		...
		if(cost > ch->practice)
		{
			sprintf(buf, "It costs you %li primal to improve your %s totem.\n\r", cost, arg1);
			send_to_char(buf, ch);
			return;
		}
		ch->pcdata->powers[improve] += 1;
		...
		send_to_char("To improve a totem, type: Totem <totem type> improve.\n\r", ch);
	return;
}
...
void do_bash(CHAR_DATA * ch, char *argument)
{
	...
	if(ch->pcdata->powers[WPOWER_SPIDER] < 4)
	{
	...
	act("$n sends you sprawling with a powerful bash!", ch, 0, victim, TO_VICT);
	act("You send $N sprawling with a powerful bash!", ch, 0, victim, TO_CHAR);
	act("$n sends $N sprawling with a powerful bash!", ch, 0, victim, TO_NOTVICT);
...
void do_burrow(CHAR_DATA * ch, char *argument)
{
  ...
  if(IS_AFFECTED(ch, AFF_SAFE))
	...
	act("$n burrows into the ground.", ch, 0, 0, TO_ROOM);
	char_from_room(ch);
	char_to_room(ch, victim->in_room);
	do_look(ch, "");
	send_to_char("You burrow out of the ground.\n\r", ch);
	act("$n burrows out of the ground.", ch, 0, 0, TO_ROOM);
	return;
}
...
void do_moonarmour(CHAR_DATA * ch, char *argument)
{
	...
	if(50 > ch->practice)
	{
		sprintf(buf, "It costs you 50 primal to create a piece of the moon.\n\r");
		send_to_char(buf, ch);
		return;
	}
	...
	act("$p appears in your hands in a blast of moon light.", ch, obj, 0, TO_CHAR);
	act("$p appears in $n's hands in a blast of moon light.", ch, obj, 0, TO_ROOM);
	return;
}
...
void do_klaive(CHAR_DATA * ch, char *argument)
{
	...
	if((obj = get_eq_char(ch, WEAR_WIELD)) != 0 && obj->pIndexData->vnum == 29519)
	{
		// greater klaive
		vnum = 29520;
		...
		if(50 > ch->practice)
		{
			send_to_char("It costs 50 primal to create the Greater Klaive.\n\r", ch);
			return;
		}
		extract_obj(obj);
		ch->practice -= 50;
...
void do_zulo( CHAR_DATA *ch, char *argument )
{
    ...
    {
	act( "You change into an enormous black beast.", ch, 0, 0, TO_CHAR );
	act( "$n changes into an enormous black beast.", ch, 0, 0, TO_ROOM );
	SET_BIT(ch->polyaff, POLY_ZULO);
	SET_BIT(ch->pcdata->stats[UNI_AFF], VAM_CHANGED);
	SET_BIT(ch->affected_by, AFF_POLYMORPH);
...
/*	send_to_char("Disciplines:",ch);
	...
	if (IS_VAMPAFF(victim, VAM_PRESENCE)) send_to_char("Presence",ch);
        ...
        if (IS_VAMPAFF(victim, VAM_PANIMAL)) send_to_char("Animalism",ch);
...
*/
		...
		send_to_char("                              Disciplines\n\r", ch);
		line(ch);
		sprintf(buf, "Animalism:    [%li]             Celerity:   [%li]             Fortitude: [%li]\n\r",
			victim->pcdata->powers[VAM_ANIM],
			victim->pcdata->powers[VAM_CELE], victim->pcdata->powers[VAM_FORT]);
		send_to_char(buf, ch);
		sprintf(buf, "Obtenebration:[%li]             Presence:   [%li]             Quietus:   [%li]\n\r",
			victim->pcdata->powers[VAM_OBTE],
			victim->pcdata->powers[VAM_PRES], victim->pcdata->powers[VAM_QUIE]);
		send_to_char(buf, ch);
		sprintf(buf, "Thaumaturgy:  [%li]             Auspex:     [%li]             Dominate:  [%li]\n\r",
			victim->pcdata->powers[VAM_THAU],
			victim->pcdata->powers[VAM_AUSP], victim->pcdata->powers[VAM_DOMI]);
		send_to_char(buf, ch);
		sprintf(buf, "Obfuscate:    [%li]             Potence:    [%li]             Protean:   [%li]\n\r",
			victim->pcdata->powers[VAM_OBFU],
			victim->pcdata->powers[VAM_POTE], victim->pcdata->powers[VAM_PROT]);
		send_to_char(buf, ch);
		sprintf(buf, "Serpentis:    [%li]             Vicissitude:[%li]             Daimoinon: [%li]\n\r",
			victim->pcdata->powers[VAM_SERP],
			victim->pcdata->powers[VAM_VICI], victim->pcdata->powers[VAM_DAIM]);
		send_to_char(buf, ch);
		sprintf(buf, "Chimerstry:   [%li]             Obeah:      [%li]             Melpominee:[%li]\n\r",
			victim->pcdata->powers[VAM_CHIM], victim->pcdata->powers[VAM_OBEA],
			victim->pcdata->powers[VAM_MELP]);
		send_to_char(buf, ch);
		sprintf(buf, "Necromancy:   [%li]                                         Thanatosis: [%li]\n\r",

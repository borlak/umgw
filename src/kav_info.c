...
#include <time.h>
#include "merc.h"
long col_str_len args((char *text));
...
void do_immune(CHAR_DATA * ch, char *argument)
{
	...
	if(!
	   (!IS_IMMUNE(ch, IMM_SLASH) && !IS_IMMUNE(ch, IMM_STAB) && !IS_IMMUNE(ch, IMM_SMASH)
	    && !IS_IMMUNE(ch, IMM_ANIMAL) && !IS_IMMUNE(ch, IMM_MISC)))
	...
	if(!
	   (!IS_IMMUNE(ch, IMM_CHARM) && !IS_IMMUNE(ch, IMM_HEAT) && !IS_IMMUNE(ch, IMM_COLD)
	    && !IS_IMMUNE(ch, IMM_LIGHTNING) && !IS_IMMUNE(ch, IMM_ACID) && !IS_IMMUNE(ch, IMM_SUMMON)
	    && !IS_IMMUNE(ch, IMM_VOODOO) && !IS_IMMUNE(ch, IMM_SLEEP) && !IS_IMMUNE(ch, IMM_DRAIN)))
	...
	if(!
	   (!IS_IMMUNE(ch, IMM_HURL) && !IS_IMMUNE(ch, IMM_BACKSTAB) && !IS_IMMUNE(ch, IMM_KICK)
	...
	if(!(IS_IMMUNE(ch, IMM_VAMPIRE) && !IS_IMMUNE(ch, IMM_STAKE) && !IS_IMMUNE(ch, IMM_SUNLIGHT)))
...
void show_spell(CHAR_DATA * ch, long dtype)
{
	char buf[MAX_INPUT_LENGTH];
...
void take_item(CHAR_DATA * ch, OBJ_DATA * obj)
{
	...
	act("You wince in pain and $p falls to the ground.", ch, obj, 0, TO_CHAR);
	act("$n winces in pain and $p falls to the ground.", ch, obj, 0, TO_ROOM);
	return;
}
...
void birth_date(CHAR_DATA * ch, bool is_self)
{
...

	{
		if(!IS_EXTRA(ch, EXTRA_BORN))
			yy += 17;
		sprintf(buf, "You are %li years, %li %s and %li %s old.\n\r", yy, mm, mstatus, dad, dstatus);
		send_to_char(buf, ch);
	}
...

	{
		...
		{
			sprintf(buf, "You are %li %s and %li %s pregnant.\n\r", mm, mstatus, dad, dstatus);
			send_to_char(buf, ch);
		}
	}
	return;
}
void other_age(CHAR_DATA * ch, long extra, bool is_preg, char *argument)
{
	char buf[MAX_STRING_LENGTH];
	char buf2[MAX_STRING_LENGTH];
	char mstatus[20];
...

	{
		if(!IS_EXTRA(ch, EXTRA_BORN))
			yy += 17;
		sprintf(buf, "Age: %li years, %li %s and %li %s old.\n\r", yy, mm, mstatus, dad, dstatus);
		send_to_char(buf, ch);
	}
...

	{
		...
		{
			sprintf(buf, "She is %li %s and %li %s pregnant.\n\r", mm, mstatus, dad, dstatus);
			send_to_char(buf, ch);
		}
	}
	return;
...
*/
	...
	if(strlen(ch->pcdata->marriage) > 1 && str_cmp(ch->pcdata->marriage, victim->name))
	...
	if(strlen(victim->pcdata->marriage) > 1 && str_cmp(victim->pcdata->marriage, ch->name))
	...
	act("You give $N permission to use xsocials on you.", ch, 0, victim, TO_CHAR);
	act("$n has given you permission to use xsocials on $m.", ch, 0, victim, TO_VICT);
	return;
}
